...
def determine_execution_state(plan, tf_results, direction):
    ...
    rr_fail = structural_rr < MIN_STRUCTURAL_RR
    if quality_fail or rr_fail:
        ...
        if rr_fail:
            reasons.append(
                f"structural R:R {structural_rr:.2f} < minimum {MIN_STRUCTURAL_RR:.2f}"
            )
...
def run_scan(active_key, symbols):
    ...
    fetched in parallel. Analysis, lifecycle persistence, ranking, and Telegram
    decisions remain serialized in the main thread so shared JSON state and
    notifications cannot race.
    """
    ...
    # Submit only data acquisition to the worker pool. Do NOT move lifecycle
    # or Telegram work into workers: those paths mutate shared JSON state and
    # must remain deterministic/serialized.
                    ...
                    if plan:
                        ...
                        if li.get("send_alert") and plan.get("status") in ("READY_MARKET", "READY_LIMIT"):
                            if TELEGRAM_ENABLED:
                                ...
                                if not sent:
                                    print(f"  ! {symbol}: Telegram alert was eligible but delivery failed.")
                            else:
    ...
    if no_trade_reasons:
        ...
        labels = {
            ...
            "STRUCTURAL_RR_BELOW_FLOOR": "Structural R:R below minimum",
            "COUNTERTREND_WITHOUT_REVERSAL_CONFIRMATION": "Countertrend without reversal confirmation",
            "SETUP_QUALITY_AND_STRUCTURAL_RR_BELOW_FLOOR": "Setup quality AND structural R:R below minimum",
