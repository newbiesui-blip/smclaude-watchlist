...
AUTO_ADD_MIN_SCORE = 90                       # unattended full-scan (GitHub Actions) only
...
#
# Trade horizon is deliberately separated from stop distance and R:R.
# A tight stop does not automatically make a trade a scalp, and a large
# structural R:R does not automatically make a trade a swing.
#
# The scanner's actual structural information is used instead:
#   - fresh momentum / fresh sweep = short-duration candidate
#   - established continuation / pullback = intraday candidate
#   - 4H/1D structural destination = swing horizon
#
# R:R remains a quality/risk metric, not a horizon classifier.

SWING_TARGET_TIMEFRAMES = {"1D", "4H"}


def classify_trade_type(setup_type, primary_target=None, tf_results=None):
    """Classify SCALP / INTRADAY / SWING from setup structure.

    Priority:
      1. A primary target sourced from 4H/1D structure -> SWING.
      2. A fresh liquidity-sweep reversal -> SCALP candidate.
      3. A fresh momentum continuation -> SCALP.
      4. Established continuation/pullback -> INTRADAY.

    This function intentionally does NOT use stop percentage or R:R.
    """
    if primary_target:
        target_tf = primary_target.get("timeframe_source")
        if target_tf in SWING_TARGET_TIMEFRAMES:
            return "SWING"

    if setup_type in ("LIQUIDITY_SWEEP_REVERSAL", "MOMENTUM_CONTINUATION"):
        return "SCALP"
...
def build_entry_plan(tf_results, direction, regime_info=None):
    """
    ...
    structural_candidates = gather_structural_targets(
        tf_results, direction, price, alignment
    )
    risk = abs(price - invalidation)
    validated_targets = build_validated_targets(
        structural_candidates, price, risk
    )
    primary_target = pick_primary_structural_target(structural_candidates)

    # Classify the trade horizon only after the real structural target is
    # known. This prevents stop-distance % and R:R from masquerading as
    # holding-time classification.
    trade_type = classify_trade_type(
        setup_type, primary_target, tf_results
    )
...
def run_scan(active_key, symbols):
    ...
    for n, symbol in enumerate(symbols, 1):
        ...
        if score >= MIN_SETUP_SCORE and direction:
            plan = build_entry_plan(tf_results, direction, regime_info)
            if plan:
