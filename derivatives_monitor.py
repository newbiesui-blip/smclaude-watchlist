...
MAX_LS_RATIO_REQUESTS = 10              # cap deep L/S requests per cycle
...
def _get_detailed(path, params, timeout=REQUEST_TIMEOUT):
    """Low-level GET with diagnostics. Never raises."""
    url = BASE_URL + path
    attempts = 0
    while True:
        try:
        ...
        if resp.status_code == 429:
            return None, {"status": "RATE_LIMITED", "http_status": 429, "path": path}
        ...
        if not isinstance(data, dict):
            return None, {"status": "INVALID_RESPONSE", "path": path}
        if data.get("retCode") != 0:
            return None, {
                "status": "API_ERROR",
                "ret_code": data.get("retCode"),
                "ret_msg": str(data.get("retMsg", ""))[:200],
                "path": path,
            }
...
def _get(path, params, timeout=REQUEST_TIMEOUT):
    """Backward-compatible GET returning only the parsed result."""
    result, _ = _get_detailed(path, params, timeout=timeout)
    return result
...
class BybitAdapter:
    ...
    def __init__(self):
        self.last_bulk_diagnostics = {"status": "NOT_RUN"}
    ...
    def fetch_bulk_tickers(self):
        ...
        """
        result, diagnostics = _get_detailed(
            "/v5/market/tickers", {"category": "linear"}
        )
        self.last_bulk_diagnostics = diagnostics
        if not result:
            return []
        rows = result.get("list", []) or []
        if not rows and diagnostics.get("status") == "OK":
            self.last_bulk_diagnostics = {
                **diagnostics,
                "status": "NO_DATA",
                "message": "Bybit returned no ticker rows",
            }
        return rows
    ...
    def fetch_open_interest(self, symbol, interval_time="15min", limit=2):
        ...
        result, diagnostics = _get_detailed("/v5/market/open-interest", {
            "category": "linear",
            "symbol": symbol,
            "intervalTime": interval_time,
            "limit": limit,
        })
        self.last_request_diagnostics[f"open_interest:{normalize_symbol(symbol)}"] = diagnostics
        if not result:
            return None
    ...
    def fetch_account_ratio(self, symbol, period="15min", limit=1):
        ...
        result, diagnostics = _get_detailed("/v5/market/account-ratio", {
            "category": "linear",
            "symbol": symbol,
            "period": period,
            "limit": limit,
        })
        self.last_request_diagnostics[f"account_ratio:{normalize_symbol(symbol)}"] = diagnostics
        if not result:
            return None
        return result.get("list", []) or []
    ...
    def fetch_snapshot(self, symbol, ticker_row=None, include_ls_ratio=True):
        ...
        ls_diagnostics = {"status": "NOT_REQUESTED"}
        if include_ls_ratio:
            try:
                ls_rows = self.fetch_account_ratio(symbol)
                ls_diagnostics = self.last_request_diagnostics.get(
                    f"account_ratio:{symbol}", {"status": "UNKNOWN"}
                )
                if ls_rows:
                    ...
                    if buy_ratio is not None and sell_ratio not in (None, 0):
                        ...
                        ls_diagnostics = {
                            **ls_diagnostics,
                            "status": "INVALID_DATA",
                            "message": "Missing/invalid buyRatio or sellRatio",
                        }
                elif ls_diagnostics.get("status") == "OK":
                    ls_diagnostics = {
                        **ls_diagnostics,
                        "status": "NO_DATA",
                        "message": "Bybit returned no account-ratio rows",
                    }
            except Exception as exc:
                ls_diagnostics = {
                    "status": "ERROR",
        ...
        snapshot = {
            ...
            "long_short_ratio_diagnostics": ls_diagnostics,
        }
        return snapshot
...
def monitor(symbols=None, adapter=None, history=None, state_store=None,
    ...
    out = {
        ...
        "bulk_fetch_ok": True,
        "source_status": {
            "bybit_bulk_tickers": "NOT_RUN",
            "bybit_long_short_ratio": "NOT_RUN",
        },
        "diagnostics": {},
    }
    ...
    try:
        ...
        out["source_status"]["bybit_bulk_tickers"] = "ERROR"
        out["diagnostics"]["bulk_fetch"] = {
            "status": "ERROR",
            "error_type": type(exc).__name__,
            "message": str(exc)[:200],
        }
    ...
    bulk_diag = getattr(adapter, "last_bulk_diagnostics", {"status": "UNKNOWN"})
    out["diagnostics"]["bulk_fetch"] = bulk_diag
    out["source_status"]["bybit_bulk_tickers"] = bulk_diag.get("status", "UNKNOWN")

    if not raw_rows:
        out["bulk_fetch_ok"] = False
        if out["source_status"]["bybit_bulk_tickers"] == "OK":
            out["source_status"]["bybit_bulk_tickers"] = "NO_DATA"
        return out
    ...
    # By default, fetch L/S for the explicitly selected target symbols.
    # Deep requests are capped so a large universe can never create an API burst.
    ls_source = include_ls_ratio_for if include_ls_ratio_for is not None else target_symbols
    ls_targets = {
        normalize_symbol(s)
        for s in list(ls_source)[:MAX_LS_RATIO_REQUESTS]
    }
    out["diagnostics"]["long_short_ratio_requests"] = {
        "requested": sorted(ls_targets),
        "cap": MAX_LS_RATIO_REQUESTS,
    }
    ...
    for symbol in target_symbols:
        try:
            ...
            if symbol in ls_targets:
                ls_diag = getattr(adapter, "last_request_diagnostics", {}).get(
                    f"account_ratio:{symbol}", {"status": "UNKNOWN"}
                )
                out["results"][symbol]["source_status"] = {
                    "bybit_ticker": "OK",
                    "bybit_long_short_ratio": ls_diag.get("status", "UNKNOWN"),
                }
        except Exception as exc:
    ...
    out["diagnostics"]["requests"] = getattr(
        adapter, "last_request_diagnostics", {}
    )
    ls_statuses = [
        r.get("source_status", {}).get("bybit_long_short_ratio")
        for r in out["results"].values()
        if "source_status" in r
    ]
    if any(status == "OK" for status in ls_statuses):
        out["source_status"]["bybit_long_short_ratio"] = "OK"
    elif ls_statuses:
        out["source_status"]["bybit_long_short_ratio"] = "DEGRADED"
    else:
        out["source_status"]["bybit_long_short_ratio"] = "NOT_REQUESTED"
...
def _run_tests():
    ...
    # --- Regression: monitor catches bulk adapter exceptions ---
    class ExplodingBulkAdapter(BybitAdapter):
        def fetch_bulk_tickers(self):
    ...
    try:
        out = monitor(
            symbols=["BTCUSDT"],
            adapter=ExplodingBulkAdapter(),
            history={},
            state_store={},
            persist=False,
        )
        check(
            "Regression: monitor catches bulk adapter exception",
            out["bulk_fetch_ok"] is False
            and out["source_status"]["bybit_bulk_tickers"] == "ERROR",
            f"out={out}",
        )
    except Exception as exc:
        check("Regression: monitor catches bulk adapter exception", False, str(exc))

    # --- Regression: default L/S requests follow selected symbols, capped at 10 ---
    class LSProbeAdapter(BybitAdapter):
        def __init__(self):
            super().__init__()
            self.requested_ls = []
        ...
        def fetch_account_ratio(self, symbol, period="15min", limit=1):
            ...
            self.requested_ls.append(sym)
            self.last_request_diagnostics[f"account_ratio:{sym}"] = {"status": "OK"}
            return [{"buyRatio": "2.0", "sellRatio": "1.0"}]

    try:
        probe = LSProbeAdapter()
        out = monitor(
        ...
        check(
            "Regression: default L/S requests follow symbols and cap at 10",
            len(probe.requested_ls) == 10
            and set(probe.requested_ls) == {f"S{i}USDT" for i in range(10)},
            f"requested={probe.requested_ls}",
        )
        check(
            "Regression: L/S ratio is populated and source status is exposed",
            out["results"]["S0USDT"]["snapshot"]["long_short_ratio"] == 2.0
            and out["results"]["S0USDT"]["source_status"]["bybit_long_short_ratio"] == "OK",
            f"result={out['results'].get('S0USDT')}",
        )
    except Exception as exc:
        check("Regression: L/S ratio is populated and source status is exposed", False, str(exc))
