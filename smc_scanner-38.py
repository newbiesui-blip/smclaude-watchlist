...
def build_entry_plan(tf_results, direction, regime_info=None, setup_score=None):
    ...
    # Invalidation is tied to the original structural geometry.
    if zone_here and setup_type == "TREND_PULLBACK":
        invalidation = find_structural_invalidation(entry, direction, zone_here)
        invalidation_reason = (
            f"Structural swing point beyond the {zone_lbl.lower()} at "
        ...
        invalidation_reason = (
            f"Nearest real swing point beyond the structural reference "
            f"{reference_entry:.6g} -- if it breaks, the structure this "
            f"setup relies on is gone."
        )
