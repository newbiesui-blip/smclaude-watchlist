...
def main():
    ...
    for plan in ready:
        ...
        # NEW SETUP is an internal lifecycle event only. Never send it to Telegram.
        # The first user-visible setup notification is emitted only when the
        # existing scanner lifecycle has advanced beyond the NEW/watch stage.
        # This keeps confirmation semantics inside smc_scanner and leaves the
        # notification/persistence boundary here as a strict visibility gate.
        if lifecycle.get("send_alert") and not lifecycle.get("is_new", False):
