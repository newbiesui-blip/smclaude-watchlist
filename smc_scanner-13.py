...
def add_to_watchlist(symbol, exchange_key, score, direction, plan,
    ...
    items.append({
        ...
        "peak_score": score, "last_warning": None, "history": [],
    })
...
def refresh_entry(entry, active_key):
    ...
    if entry["status"] == "triggered":
        ...
        warning = None
        if entry.get("max_r", 0.0) >= REENTRY_MIN_NEW_R and peak > 0 and entry["current_score"] <= peak * 0.80:
            warning = (f"⚠️ EARLY REVERSAL WARNING: {symbol} {direction} reached +{entry['max_r']:.2f}R "
                       f"but live score fell to {entry['current_score']:.0f} from peak {peak:.0f}. "
                       "Protect/exit; no same-direction re-entry until new structure forms.")
