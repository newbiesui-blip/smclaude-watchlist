...
def _be_state(
    ...
    direction = _direction({"direction": "BULLISH"})  # replaced below
    # Infer direction from price relation only as a fallback; this branch is
    # not used for the primary eligibility decision.
    if current_price > entry_price and current_price > stop:
        direction = "BULLISH"
    elif current_price < entry_price and current_price < stop:
        direction = "BEARISH"
...
def _partial_state(
    ...
    if (
        ...
        and first_r is not None
        and current_r >= first_r
    ):
        return {
...
def build_position_intelligence(
    ...
    be = _be_state(current_r, stop, entry_price, current_price)
...
def format_position_intelligence(snapshot: Dict[str, Any]) -> str:
    ...
    if pnl is not None:
        lines.append(
            f"Unrealized P&L: {pnl:+.4f} ({pnl_source})"
        )
    ...
    if sl_distance:
        ...
        lines.append(
            f"SL distance: {sl_distance['price']:.8g}{pct_text}"
        )

    be = snapshot.get("break_even", {})
    lines.append(
        f"BE: {be.get('status', 'UNKNOWN')} — {be.get('reason', '')}"
    )
...
def run_tests():
    ...
    _test("current R is preserved", snap["current_r"] == 1.0)
    ...
    calculated["exchange_avg_price"] = 100.0
    calculated["exchange_position_amount"] = 2.0
    calculated["invalidation"] = 90.0
