...
def _format_alert(symbol, used, score, direction, plan, is_new=True, reason=None):
    ...
    header = (
        ...
        if is_new
        else f"🔄 ACTIVE SETUP UPDATE: {symbol} {direction} [{plan.get('trade_type', 'INTRADAY')}]"
    )
...
def _sync_watchlist(active_key):
    ...
    items = [
        ...
        if not (
            ...
            and scanner._hours_from_now(
                it.get("invalidated_at") or it.get("expired_at")
    ...
    for it in items:
        ...
        # sync_position() returns a dictionary. Never compare it to old constants.
        sync_result = bingx.sync_position(it)
        sync_state = str((sync_result or {}).get("state", "UNKNOWN")).upper()
        ...
        if sync_state == "OPEN":
            ...
            try:
                ...
                scanner._log(
                    it,
                    f"Active refresh error, exchange state preserved: {exc}",
                )
    ...
    for n, symbol in enumerate(symbols, 1):
        ...
        try:
            ...
            exec_state = scanner.determine_execution_state(
                plan, tf_results, direction
            )
            plan.update(exec_state)

            lifecycle = scanner.update_setup_lifecycle(
                symbol, plan, scan_cycle
            )
            plan["lifecycle_info"] = lifecycle

            # Derivatives are context only. They never alter SMC fields.
            ...
            qualifying.append(
                (symbol, tf_results, used, score, direction, plan)
            )
...
def main():
    ...
    if ready:
        ...
        for rank, plan in enumerate(ready[:TOP_ACTIONABLE_TO_PRINT], 1):
            print(
                f"{rank}. {plan.get('symbol', '?')} "
                f"{plan.get('direction', '?')} {plan.get('status')} | "
    ...
    # exchange-gated triggered positions. Do not call check_watchlist().
