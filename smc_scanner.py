...
def is_ready_now(plan):
    """
    True if this setup is immediately actionable -- price is already
    inside the entry zone (market entry) or a breakout retest is already
    underway (a clean, close limit level right there). False for a
    genuinely-still-approaching setup (mode "WAIT FOR PULLBACK"), which
    just means price hasn't reached the zone yet at all.
    """
    mode = plan.get("mode", "")
    return mode.startswith("PULLBACK") or mode.startswith("WAIT FOR BREAKOUT RETEST")
