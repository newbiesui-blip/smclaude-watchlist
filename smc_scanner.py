...
# --- BingX private position synchronization (READ-ONLY) ---
BINGX_API_KEY = os.environ.get("BINGX_API_KEY", "")
BINGX_SECRET_KEY = os.environ.get("BINGX_SECRET_KEY", os.environ.get("BINGX_API_SECRET", ""))
BINGX_SYNC_ENABLED = bool(BINGX_API_KEY and BINGX_SECRET_KEY)
BINGX_SYNC_LOOKBACK_HOURS = 72
BINGX_SYNC_CACHE_SECONDS = 45
BINGX_SYNC_RECV_WINDOW = 5000
BINGX_SYNC_CACHE = {"ts": 0.0, "positions": None}


def _bingx_signed_get(path, params=None):
    if not BINGX_SYNC_ENABLED:
        return None
    params = dict(params or {})
    params["timestamp"] = int(time.time() * 1000)
    params.setdefault("recvWindow", BINGX_SYNC_RECV_WINDOW)
    canonical = "&".join(f"{k}={params[k]}" for k in sorted(params))
    signature = hmac.new(BINGX_SECRET_KEY.encode(), canonical.encode(), hashlib.sha256).hexdigest()
    headers = {**HEADERS, "X-BX-APIKEY": BINGX_API_KEY, "X-SOURCE-KEY": "BX-AI-SKILL"}
    r = requests.get(f"https://open-api.bingx.com{path}?{canonical}&signature={signature}", headers=headers, timeout=8)
    r.raise_for_status()
    payload = r.json()
    if payload.get("code") not in (0, "0"):
        raise RuntimeError(f"BingX private API {path}: {payload.get('code')} {payload.get('msg', '')}")
    return payload.get("data")


def _bingx_open_positions():
    if not BINGX_SYNC_ENABLED:
        return None
    now = time.time()
    if BINGX_SYNC_CACHE["positions"] is not None and now - BINGX_SYNC_CACHE["ts"] < BINGX_SYNC_CACHE_SECONDS:
        return BINGX_SYNC_CACHE["positions"]
    data = _bingx_signed_get("/openApi/swap/v2/user/positions")
    positions = data if isinstance(data, list) else []
    BINGX_SYNC_CACHE.update({"ts": now, "positions": positions})
    return positions
...
def _bingx_history(symbol, start_ms):
    end_ms = int(time.time() * 1000)
    out = {"position_history": [], "orders": [], "fills": [], "force_orders": []}
    try:
        data = _bingx_signed_get("/openApi/swap/v1/trade/positionHistory", {"symbol": symbol, "startTs": start_ms, "endTs": end_ms, "pageIndex": 1, "pageSize": 100})
        out["position_history"] = (data or {}).get("list", []) if isinstance(data, dict) else []
    except Exception as e:
        print(f"(!) BingX positionHistory unavailable for {symbol}: {e}")
    try:
        data = _bingx_signed_get("/openApi/swap/v2/trade/allOrders", {"symbol": symbol, "startTime": start_ms, "endTime": end_ms, "limit": 100})
        out["orders"] = data if isinstance(data, list) else []
    except Exception as e:
        print(f"(!) BingX order history unavailable for {symbol}: {e}")
    try:
        data = _bingx_signed_get("/openApi/swap/v2/trade/allFillOrders", {"symbol": symbol, "limit": 100, "startTime": start_ms, "endTime": end_ms})
        out["fills"] = data if isinstance(data, list) else []
    except Exception as e:
        print(f"(!) BingX fill history unavailable for {symbol}: {e}")
    try:
        data = _bingx_signed_get("/openApi/swap/v2/trade/forceOrders", {"symbol": symbol, "startTime": start_ms, "endTime": end_ms, "limit": 100})
        out["force_orders"] = data if isinstance(data, list) else []
    except Exception as e:
        print(f"(!) BingX force-order history unavailable for {symbol}: {e}")
    return out


def _extract_realized_pnl(item):
    """Best-effort realized PnL extraction across BingX history payload variants."""
    for key in ("realizedPnl", "realizedPnL", "realizedProfit", "profit", "pnl", "closedPnl"):
        value = item.get(key)
...
def _pick_closed_bingx_event(entry, history):
    expected = entry.get("direction", "")
    trigger_ms = _parse_iso_ms(entry.get("triggered_at")) or 0
    candidates = []
    for item in history.get("position_history", []):
        side = _norm_side(item.get("positionSide") or item.get("side") or item.get("orderSide"))
        if side and side != expected:
            continue
        close_ms = _to_int(item.get("closeTime") or item.get("updateTime") or item.get("time"))
        open_ms = _to_int(item.get("openTime") or item.get("open_time"))
        if close_ms and close_ms >= trigger_ms - 5 * 60 * 1000 and (not open_ms or open_ms <= close_ms):
            candidates.append((close_ms, "POSITION_HISTORY", item))
    if candidates:
        return max(candidates, key=lambda x: x[0])
    close_types = {"STOP_MARKET", "STOP", "TAKE_PROFIT_MARKET", "TAKE_PROFIT"}
    for item in history.get("orders", []):
        if str(item.get("status", "")).upper() != "FILLED":
            continue
        side = _norm_side(item.get("positionSide"))
        if side and side != expected:
            continue
        t = _to_int(item.get("updateTime") or item.get("time"))
        if not t or t < trigger_ms - 5 * 60 * 1000:
            continue
        typ = str(item.get("type", "")).upper()
        order_side = str(item.get("side", "")).upper()
        closes = (expected == "BULLISH" and order_side == "SELL") or (expected == "BEARISH" and order_side == "BUY")
        if typ in close_types and closes:
            candidates.append((t, "ORDER", item))
    if candidates:
        return max(candidates, key=lambda x: x[0])
    for item in history.get("force_orders", []):
        t = _to_int(item.get("updateTime") or item.get("time"))
        if t and t >= trigger_ms - 5 * 60 * 1000:
            return t, "FORCE_ORDER", item
    fills = []
    for item in history.get("fills", []):
        t = _to_int(item.get("time") or item.get("tradeTime"))
        if not t or t < trigger_ms - 5 * 60 * 1000:
            continue
        order_side = str(item.get("side", "")).upper()
        closes = (expected == "BULLISH" and order_side == "SELL") or (expected == "BEARISH" and order_side == "BUY")
        if closes:
            try: realized = abs(float(item.get("realizedPnl", 0) or 0))
            except Exception: realized = 0.0
            fills.append((t, "FILL", item, realized))
    if fills:
        fills.sort(key=lambda x: (x[0], x[3]), reverse=True)
        t, source, item, _ = fills[0]
        return t, source, item
    return None


def _bingx_position_sync(entry):
    """Return False=open, True=closed, None=unknown. Read-only."""
    if not BINGX_SYNC_ENABLED or entry.get("status") != "triggered":
        return None
    symbol, direction = entry["symbol"], entry["direction"]
    try:
        positions = _bingx_open_positions() or []
        same, opposite = [], []
        for pos in positions:
            if str(pos.get("symbol", "")).upper() != symbol.upper():
                continue
            side = _norm_side(pos.get("positionSide"))
            amount = abs(float(pos.get("positionAmt", 0) or 0))
            if amount <= 0: continue
            (same if side == direction else opposite).append(pos)
        if same:
            pos = same[0]
            pos_id = str(pos.get("positionId", ""))
            actual_avg = float(pos.get("avgPrice", 0) or 0)
            bound_id = str(entry.get("exchange_position_id") or "")

            # Never attach an old tracker to a newly opened same-side position.
            if bound_id and pos_id and bound_id != pos_id:
                same = []
            elif not bound_id:
                ep = float(entry.get("entry_price") or 0)
                zl = float(entry.get("zone_low") or 0)
                zh = float(entry.get("zone_high") or 0)
                tolerance = max(ep * 0.015, abs(zh - zl) * 0.50)
                if ep and actual_avg and abs(actual_avg - ep) > tolerance and not (zl - tolerance <= actual_avg <= zh + tolerance):
                    entry["exchange_sync_status"] = "NOT_MATCHED"
                    entry["exchange_sync_at"] = datetime.now(timezone.utc).isoformat()
                    return None

            if same:
                new_amt = abs(float(pos.get("positionAmt", 0) or 0))
                old_amt = entry.get("exchange_position_amt")
                entry["exchange_sync_status"] = "OPEN"
                entry["exchange_position_id"] = pos_id
                entry["exchange_avg_price"] = actual_avg
                entry["exchange_position_amt"] = new_amt
                entry["exchange_unrealized_pnl"] = float(pos.get("unrealizedProfit", 0) or 0)
                entry["exchange_liquidation_price"] = float(pos.get("liquidationPrice", 0) or 0)
                entry["exchange_sync_at"] = datetime.now(timezone.utc).isoformat()
                if old_amt is not None and new_amt < float(old_amt) - 1e-12:
                    entry["exchange_partial_close_detected"] = True
                    entry["exchange_partial_close_from"] = float(old_amt)
                    entry["exchange_partial_close_to"] = new_amt
                    _log(entry, f"BingX detected partial reduction: {float(old_amt):.8g} -> {new_amt:.8g}. Position remains OPEN.")
                return False
        if opposite:
            entry["exchange_sync_status"] = "REVERSED"
            entry["exchange_opposite_position"] = True
            entry["exchange_sync_at"] = datetime.now(timezone.utc).isoformat()
            entry["position_exit_reason"] = "REVERSED"
            entry["position_lifecycle"] = "CLOSED"
            entry["status"] = "closed"
            entry["exchange_closed_at"] = datetime.now(timezone.utc).isoformat()
            _log(entry, f"BingX confirms the {direction} position is closed and an opposite position is now open. Scanner stopped tracking the old position.")
            return True
        trigger_ms = _parse_iso_ms(entry.get("triggered_at")) or int((time.time() - 3600) * 1000)
        history = _bingx_history(symbol, max(0, trigger_ms - BINGX_SYNC_LOOKBACK_HOURS * 3600 * 1000))
        event = _pick_closed_bingx_event(entry, history)
        if not event:
            entry["exchange_sync_status"] = "NOT_FOUND"
            entry["exchange_sync_at"] = datetime.now(timezone.utc).isoformat()
            return None
        close_ms, source, item = event
        typ = str(item.get("type", "")).upper()
        if source == "FORCE_ORDER": reason = "LIQUIDATION"
        elif typ in ("STOP_MARKET", "STOP"): reason = "STOP_LOSS"
        elif typ in ("TAKE_PROFIT_MARKET", "TAKE_PROFIT"): reason = "TAKE_PROFIT"
        else: reason = "EXCHANGE_CLOSED"
        close_price = item.get("avgClosePrice") or item.get("averageClosePrice") or item.get("closePrice") or item.get("avgPrice") or item.get("price") or item.get("tradePrice")
        try: close_price = float(close_price) if close_price not in (None, "") else None
        except Exception: close_price = None
        realized_pnl = _extract_realized_pnl(item)
        entry["exchange_realized_pnl"] = realized_pnl
        if reason == "TAKE_PROFIT" or (realized_pnl is not None and realized_pnl > 0):
            entry["position_exit_reason"] = "TAKE_PROFIT" if reason == "TAKE_PROFIT" else "CLOSED_IN_PROFIT"
        entry["exchange_sync_status"] = "CLOSED"
        entry["exchange_closed_at"] = datetime.fromtimestamp(close_ms / 1000, tz=timezone.utc).isoformat()
        entry["exchange_close_source"] = source
        entry["exchange_close_order_id"] = str(item.get("orderId", item.get("positionId", "")))
        ...
        if reason == "STOP_LOSS": entry["invalidated_at"] = entry["exchange_closed_at"]
        close_text = f" at {close_price:.6g}" if close_price is not None else ""
        _log(entry, f"BingX confirms position CLOSED{close_text}. Source={source}; reason={reason}. No further price-based R/TP/health updates will be generated.")
        return True
    except Exception as e:
        entry["exchange_sync_status"] = "ERROR"
        entry["exchange_sync_error"] = str(e)[:300]
        entry["exchange_sync_at"] = datetime.now(timezone.utc).isoformat()
        print(f"(!) BingX position sync failed for {symbol}: {e}")
        return None
...
POSITION_TRACKER_VERSION = 1  # read-only/paper position lifecycle tracker
...
def add_to_watchlist(symbol, exchange_key, score, direction, plan,
    ...
    items.append({
        ...
        "position_tracker_version": POSITION_TRACKER_VERSION,
        "position_opened_at": None, "position_lifecycle": "PENDING",
        "position_exit_reason": None, "position_age_minutes": 0.0,
        "position_targets": [], "tp_status": [], "tp_hit_at": {},
        "exchange_sync_status": "UNSYNCED", "exchange_sync_at": None,
        "exchange_position_id": None, "exchange_avg_price": None, "exchange_position_amt": None,
        "exchange_unrealized_pnl": None, "exchange_realized_pnl": None, "exchange_liquidation_price": None,
        "exchange_closed_at": None, "exchange_close_price": None, "exchange_close_source": None,
        "exchange_close_order_id": None, "exchange_sync_error": None,
        "exchange_partial_close_detected": False,
        "history": [],
    })
...
def _notify_status_change(entry, prev_status):
    ...
    if new_status == "triggered":
        ...
        if entry.get("exchange_sync_status") == "CLOSED":
            cp = entry.get("exchange_close_price")
            cpt = f" at {cp:.6g}" if cp is not None else ""
            lines.append(f"BingX confirmation: STOP_LOSS closure{cpt} ({entry.get('exchange_close_source', 'exchange')} evidence).")
            lines.append("No further R/TP/health updates will be generated for this trade.")
        ...
        reason = entry.get("position_exit_reason", "EXCHANGE_CLOSED")
        cp = entry.get("exchange_close_price")
        cpt = f" at {cp:.6g}" if cp is not None else ""
        pnl = entry.get("exchange_realized_pnl")
        if reason == "TAKE_PROFIT" or (pnl is not None and pnl > 0):
            title = f"\U0001F3AF POSITION CLOSED IN PROFIT: {symbol} {direction} [{trade_type}]"
        else:
            title = f"\U0001F534 POSITION CLOSED: {symbol} {direction} [{trade_type}]"
        lines = [title, f"Reason: {reason}{cpt}"]
        if pnl is not None:
            lines.append(f"Realized PnL: {pnl:+.6g}")
        lines += [
            f"BingX confirmation: {entry.get('exchange_close_source', 'account state')}",
            "Position is permanently closed in the tracker.",
            "No further R/TP/health updates will be generated for this trade.",
        ]
    else:
...
def _position_age_minutes(entry):
    ts = entry.get("triggered_at") or entry.get("position_opened_at")
    if not ts:
        return 0.0
    try:
        return max(0.0, (datetime.now(timezone.utc) - datetime.fromisoformat(ts)).total_seconds() / 60.0)
    except Exception:
        return 0.0


def _price_reached_target(direction, price, target_price):
    return price >= target_price if direction == "BULLISH" else price <= target_price


def _position_tracker_update(entry, price):
    """Update read-only/paper position lifecycle metrics for a triggered entry."""
    if entry.get("status") != "triggered":
        return []
    entry.setdefault("position_tracker_version", POSITION_TRACKER_VERSION)
    entry.setdefault("position_opened_at", entry.get("triggered_at"))
    entry.setdefault("position_lifecycle", "ACTIVE")
    entry.setdefault("position_exit_reason", None)
    entry.setdefault("position_age_minutes", 0.0)
    entry.setdefault("tp_status", [])
    entry.setdefault("tp_hit_at", {})
    entry.setdefault("position_targets", [])

    # Freeze the original TP map on first triggered refresh so dynamic target
    # recalculation cannot erase evidence that a target was already reached.
    if not entry["position_targets"]:
        source_targets = entry.get("targets") or []
        entry["position_targets"] = [
            {"price": float(t["price"]), "r": float(t.get("r", 0.0)),
             "label": t.get("label", f"TP{i+1}"),
             "target_type": t.get("target_type"),
             "timeframe_source": t.get("timeframe_source")}
            for i, t in enumerate(source_targets[:3])
            if t.get("price") is not None
        ]

    statuses = list(entry.get("tp_status") or [])
    while len(statuses) < len(entry["position_targets"]):
        statuses.append("PENDING")

    newly_hit = []
    for i, target in enumerate(entry["position_targets"]):
        if statuses[i] == "HIT":
            continue
        if _price_reached_target(entry["direction"], price, target["price"]):
            statuses[i] = "HIT"
            entry["tp_hit_at"][f"TP{i+1}"] = _utc_now_iso()
            newly_hit.append((i + 1, target))

    entry["tp_status"] = statuses
    entry["position_age_minutes"] = round(_position_age_minutes(entry), 1)
    hit_count = sum(1 for x in statuses if x == "HIT")
    entry["position_lifecycle"] = (
        "TP3_HIT" if hit_count >= 3 else
        "TP2_HIT" if hit_count == 2 else
        "TP1_HIT" if hit_count == 1 else "ACTIVE"
    )
    return newly_hit


def _notify_position_target_hits(entry, newly_hit):
    if not TELEGRAM_ENABLED or not newly_hit:
        return
    lines = [
        f"🎯 POSITION TARGET HIT: {entry['symbol']} {entry['direction']} [{entry.get('trade_type', 'INTRADAY')}]",
        f"Lifecycle: {entry.get('position_lifecycle', 'ACTIVE')}",
        f"Current R: {entry.get('current_r', 0.0):+.2f}R | Max R: {entry.get('max_r', 0.0):+.2f}R",
        f"Position age: {entry.get('position_age_minutes', 0.0):.0f} min",
        "Existing position only -- NOT a new entry signal.",
    ]
    for n, target in newly_hit:
        lines.append(f"TP{n} reached: {target['price']:.6g} (~{target.get('r', 0.0):.2f}R)")
    lines.append("DO NOT re-enter from this alert.")
    send_telegram_message("\n".join(lines))


def _position_tracker_summary(entry):
    lifecycle = entry.get("position_lifecycle", "ACTIVE")
    age = entry.get("position_age_minutes", 0.0)
    statuses = entry.get("tp_status") or []
    tp = " ".join(f"TP{i+1}={'✅' if st == 'HIT' else '⏳'}" for i, st in enumerate(statuses)) if statuses else "TPs=not captured"
    return f"lifecycle={lifecycle} | age={age:.0f}m | {tp}"
...
def refresh_entry(entry, active_key):
    ...
    # Reconcile with BingX before any candle-based tracking. This catches an
    # intrabar SL/TP that a 15-minute snapshot can miss.
    if entry.get("status") == "triggered" and BINGX_SYNC_ENABLED:
        sync_result = _bingx_position_sync(entry)
        if sync_result is True:
            return
        if sync_result is None:
            # Exchange is the source of truth when private BingX sync is enabled.
            # Never turn an unknown/stale local tracker into a false ACTIVE update.
            entry["last_note"] = "BingX could not confirm this position as open or closed; active-position updates are suppressed."
            return
    ...
    if entry["status"] == "pending":
        ...
        if any_leg_touched:
            ...
            entry["position_tracker_version"] = POSITION_TRACKER_VERSION
            entry["position_opened_at"] = entry["triggered_at"]
            entry["position_lifecycle"] = "ACTIVE"
            entry["position_exit_reason"] = None
            entry["position_age_minutes"] = 0.0
            entry["tp_status"] = []
            entry["tp_hit_at"] = {}
            entry["position_targets"] = []
            entry["exchange_sync_status"] = "UNSYNCED"
            entry["exchange_sync_at"] = None
            entry["exchange_position_id"] = None
            entry["exchange_position_amt"] = None
            entry["exchange_closed_at"] = None
            entry["exchange_close_price"] = None
    ...
    if entry["status"] == "triggered":
        ...
        if breached:
            ...
            entry["position_lifecycle"] = "CLOSED"
            entry["position_exit_reason"] = "STOP_LOSS"
            entry["position_age_minutes"] = round(_position_age_minutes(entry), 1)
        ...
        newly_hit = _position_tracker_update(entry, price)
        _notify_position_target_hits(entry, newly_hit)
...
def check_watchlist(active_key):
    ...
    items = [it for it in items if not (
        it.get("dead_reported") and it.get("status") in ("invalidated", "expired", "closed")
        and _hours_from_now(it.get("invalidated_at") or it.get("expired_at") or it.get("exchange_closed_at")) >= REENTRY_HISTORY_HOURS
    )]
    ...
    for i, it in enumerate(items):
        ...
        it.setdefault("position_tracker_version", POSITION_TRACKER_VERSION)
        it.setdefault("position_opened_at", it.get("triggered_at"))
        it.setdefault("position_lifecycle", "ACTIVE" if it.get("status") == "triggered" else "PENDING")
        it.setdefault("position_exit_reason", None)
        it.setdefault("position_age_minutes", 0.0)
        it.setdefault("position_targets", [])
        it.setdefault("tp_status", [])
        it.setdefault("tp_hit_at", {})
        it.setdefault("exchange_sync_status", "UNSYNCED")
        it.setdefault("exchange_sync_at", None)
        it.setdefault("exchange_position_id", None)
        it.setdefault("exchange_avg_price", None)
        it.setdefault("exchange_position_amt", None)
        it.setdefault("exchange_unrealized_pnl", None)
        it.setdefault("exchange_realized_pnl", None)
        it.setdefault("exchange_liquidation_price", None)
        it.setdefault("exchange_closed_at", None)
        it.setdefault("exchange_close_price", None)
        it.setdefault("exchange_close_source", None)
        it.setdefault("exchange_close_order_id", None)
        it.setdefault("exchange_sync_error", None)
        it.setdefault("exchange_partial_close_detected", False)
        ...
        try:
            ...
            if it["status"] != prev_status:
                ...
                if it["status"] in ("invalidated", "expired", "closed"):
        ...
        if it["status"] == "triggered":
            print(f"    >> POSITION: {_position_tracker_summary(it)}")
        if it["targets"]:
