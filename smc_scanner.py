...
REQUEST_DELAY = 0.25                          # global minimum interval between exchange request starts
SCAN_WORKERS = 8                               # bounded I/O concurrency for full-market scans

# Global request-start limiter shared by all scan workers.
_REQUEST_RATE_LOCK = threading.Lock()
_REQUEST_NEXT_TIME = 0.0

def _pace_exchange_request():
    """Globally pace exchange request starts across concurrent workers."""
    global _REQUEST_NEXT_TIME
    with _REQUEST_RATE_LOCK:
        now = time.monotonic()
        wait = max(0.0, _REQUEST_NEXT_TIME - now)
        _REQUEST_NEXT_TIME = max(now, _REQUEST_NEXT_TIME) + REQUEST_DELAY
    if wait > 0:
        time.sleep(wait)
...
MIN_TRADE_SCORE = 80                          # READY-tier evidence floor; WAITING setups may score 60-79
MIN_READY_SCORE = MIN_TRADE_SCORE             # explicit alias: score is a READY gate, not a WAIT gate
...
def _zone_distance_atr(price, zone, atr):
    mid = (zone["low"] + zone["high"]) / 2
    if atr and atr > 0:
        return abs(price - mid) / atr
    return abs(price - mid) / max(abs(mid), 1e-9)


def _best_directional_zone(entry, direction, max_atr=1.5):
    """Closest valid unmitigated zone for the requested direction."""
    if not entry:
        return None
    want = "bullish" if direction == "BULLISH" else "bearish"
    zones = list((entry.get("bullish_zones") if want == "bullish" else entry.get("bearish_zones")) or [])
    zones = [z for z in zones if not z.get("mitigated", False)]
    if not zones:
        return None
    atr = compute_atr(entry.get("df"))
    price = float(entry["price"])
    ranked = sorted(zones, key=lambda z: _zone_distance_atr(price, z, atr))
    if max_atr is not None and atr and atr > 0:
        ranked = [z for z in ranked if _zone_distance_atr(price, z, atr) <= max_atr]
    return ranked[0] if ranked else None


def _recent_supporting_sweep(entry, direction, max_age=5):
    sweep = entry.get("recent_sweep") if entry else None
    if not sweep:
        return None
    want = "bullish" if direction == "BULLISH" else "bearish"
    if sweep.get("direction") != want:
        return None
    age = entry.get("current_index", 0) - sweep.get("index", 0)
    return sweep if 0 <= age <= max_age else None


def score_setup(tf_results):
    """Clean 0-100 structural evidence score; regime is deliberately separate.

    35 HTF agreement + 25 directional location + 25 fresh structure +
    15 recent liquidity support. The same break is not scored twice.
    """
    entry = tf_results.get(ENTRY_TF)
    if entry is None or entry.get("bias") == "RANGING":
        return 0, None
    direction = entry["bias"]
    score = 0

    for tf, points in (("1D", 20), ("4H", 15)):
        r = tf_results.get(tf)
        if r and r.get("bias") == direction:
            score += points

    zone = _best_directional_zone(entry, direction, max_atr=1.5)
    if zone:
        d_atr = _zone_distance_atr(entry["price"], zone, compute_atr(entry.get("df")))
        if d_atr <= 0.50:
            score += 25
        elif d_atr <= 1.00:
            score += 18
    ...
    # Fresh structure is scored once per distinct timeframe evidence source:
    # entry-TF confirmation is 15 points; a fresh aligned HTF break contributes
    # 10 more.  This preserves a true 25-point bucket without double-counting
    # the same event through setup-type classification.
    if entry.get("fresh_break"):
        score += 15
    for tf in HTF_TFS:
        r = tf_results.get(tf)
        if r and r.get("fresh_break") and r.get("bias") == direction:
            score += 10
            break

    if _recent_supporting_sweep(entry, direction, max_age=5):
        score += 15
...
def check_reversal_evidence(tf_results, direction):
    ...
    for tf in ["4H", "1H"]:
        ...
        sweep_age = (r.get("current_index", 0) - sweep.get("index", 0)) if sweep else None
        event_sweep_gap = (abs(last_event.get("index", 0) - sweep.get("index", 0))
                           if last_event and sweep else None)
        if (fresh and last_event and last_event["direction"] == dir_word
                and sweep and sweep["direction"] == dir_word
                and sweep_age is not None and 0 <= sweep_age <= 5
                and event_sweep_gap is not None and event_sweep_gap <= SWEEP_REVERSAL_MAX_BAR_GAP):
            return True, (f"{tf} shows a recent liquidity sweep followed by a fresh "
...
def score_setup_with_regime(tf_results):
    """Return structural evidence score plus explicit regime context.

    Regime penalties are no longer hidden inside the evidence score. This
    keeps MIN_TRADE_SCORE meaningful and makes countertrend/range/conflict
    decisions auditable through regime_info.
    """
    score, direction = score_setup(tf_results)
    if direction is None:
        return score, direction, None
    return score, direction, evaluate_regime(tf_results, direction)


STRUCT_BUFFER_PCT = 0.15   # extra cushion beyond the structural stop point, in %


def _structural_buffer(entry, base):
    """Breathing room scaled to volatility, with the existing % floor."""
    atr = compute_atr(entry.get("df")) if entry else None
    return max(abs(base) * STRUCT_BUFFER_PCT / 100, (atr * 0.20) if atr and atr > 0 else 0)


def find_structural_invalidation(entry, direction, zone):
    """Use the nearest valid structural invalidation and record its source TF."""
    low, high = zone["low"], zone["high"]
    htf = (entry.get("_htf_invalidation_low_candidates") if direction == "BULLISH"
           else entry.get("_htf_invalidation_high_candidates")) or []
    if direction == "BULLISH":
        local = [p for p in entry.get("swing_low_prices", []) if p <= low * 1.001]
        # Prefer the closest valid structural point, but allow 1H structure
        # when it is the actual invalidation anchor.
        candidates = [(p, ENTRY_TF) for p in local] + [(p, "1H") for p in htf]
        base, source_tf = max(candidates, key=lambda x: x[0]) if candidates else (low, ENTRY_TF)
        entry["_last_invalidation_timeframe"] = source_tf
        return min(base - _structural_buffer(entry, base), entry["price"] * 0.999)
    local = [p for p in entry.get("swing_high_prices", []) if p >= high * 0.999]
    candidates = [(p, ENTRY_TF) for p in local] + [(p, "1H") for p in htf]
    base, source_tf = min(candidates, key=lambda x: x[0]) if candidates else (high, ENTRY_TF)
    entry["_last_invalidation_timeframe"] = source_tf
    return max(base + _structural_buffer(entry, base), entry["price"] * 1.001)


def find_structural_invalidation_no_zone(entry, direction, price):
    if direction == "BULLISH":
        htf = [p for p in (entry.get("_htf_invalidation_low_candidates") or []) if p < price]
        local = [p for p in entry.get("swing_low_prices", []) if p < price]
        if htf:
            base, source_tf = max(htf), "1H"
        elif local:
            base, source_tf = max(local), ENTRY_TF
        else:
            base, source_tf = float(entry["df"]["low"].tail(20).min()), ENTRY_TF
        entry["_last_invalidation_timeframe"] = source_tf
        return min(base - _structural_buffer(entry, base), price * 0.999)
    htf = [p for p in (entry.get("_htf_invalidation_high_candidates") or []) if p > price]
    local = [p for p in entry.get("swing_high_prices", []) if p > price]
    if htf:
        base, source_tf = min(htf), "1H"
    elif local:
        base, source_tf = min(local), ENTRY_TF
    else:
        base, source_tf = float(entry["df"]["high"].tail(20).max()), ENTRY_TF
    entry["_last_invalidation_timeframe"] = source_tf
    return max(base + _structural_buffer(entry, base), price * 1.001)
...
def _nearest_swing_beyond(tf_data, direction, price):
    ...
    if tf_data is None:
        return None
    prices = tf_data.get("swing_high_prices", []) if direction == "BULLISH" else tf_data.get("swing_low_prices", [])
...
def _nearest_zone_edge_beyond(tf_data, direction, price):
    ...
    if tf_data is None:
        return None
    zones = tf_data.get("bearish_zones", []) if direction == "BULLISH" else tf_data.get("bullish_zones", [])
...
def compute_setup_quality(setup_type, regime_info, structural_rr, structural_rr_quality,
    ...
    30 regime/context + 35 actionable reward/risk + 35 target quality.
    The distant PRIMARY structural target may describe thesis potential, but
    setup quality is anchored to the nearest actionable target so a huge
    theoretical target cannot rescue a poor first objective.  Target quality
    is tier-based and therefore no longer reuses R:R as a second score.
    Setup type is classification only, not an additional quality bonus.
    """
    regime_component = {
        "TREND_ALIGNED": 30,
        "REVERSAL": 27,
    ...
    # Use TP1 / nearest validated objective for actionable reward quality.
    # structural_rr remains a separate thesis-level diagnostic and hard gate.
    actionable_rr = validated_targets[0]["r"] if validated_targets else 0.0
    rr_component = min(max(actionable_rr, 0.0), 4.0) / 4.0 * 35

    if not validated_targets:
        target_component = 0
    else:
        nearest = validated_targets[0]
        tier_quality = TIER_BASE_QUALITY.get(nearest["target_type"], 30)
        target_component = (15 if nearest["target_type"] != "INTERNAL_LIQUIDITY" else 5) + \
                           (tier_quality / 100 * 20)
    return round(min(100, regime_component + rr_component + target_component))
...
def build_entry_plan(tf_results, direction, regime_info=None, setup_score=None):
    ...
    if not isinstance(entry, dict) or entry.get("price") is None:
        return None
    price = float(entry["price"])
    # Use the same directional zone that execution will actually use.
    # The old code anchored HTF invalidation to the globally-nearest zone,
    # which could be the wrong side of the market.
    directional_zone = _best_directional_zone(entry, direction, max_atr=1.5)
    zone = directional_zone

    # Clear per-plan scratch metadata so a reused tf_results object cannot
    # leak an invalidation source from a previous candidate.
    entry.pop("_htf_invalidation_low_candidates", None)
    entry.pop("_htf_invalidation_high_candidates", None)
    entry["_last_invalidation_timeframe"] = ENTRY_TF

    h1 = tf_results.get("1H")
    if isinstance(h1, dict):
        if direction == "BULLISH":
            anchor = zone["low"] if zone else price
            entry["_htf_invalidation_low_candidates"] = [p for p in h1.get("swing_low_prices", []) if p < anchor]
        else:
            anchor = zone["high"] if zone else price
            entry["_htf_invalidation_high_candidates"] = [p for p in h1.get("swing_high_prices", []) if p > anchor]
    ...
    # ------------------------------------------------------------------
    # TWO PRICE STATES: THESIS/PLAN vs CURRENT EXECUTION
    # ------------------------------------------------------------------
    # A strong pullback setup can be valid even when the current market
    # price is a poor place to enter. Gemini's NEAR example is exactly this:
    # current-price R:R was poor, while the planned limit zone had strong
    # structural R:R. The scanner must not destroy the setup merely because
    # price has not reached the intended execution zone yet.
    if zone_here:
        planned_entry = (zone_here["low"] + zone_here["high"]) / 2
    elif setup_type == "TREND_PULLBACK":
        planned_entry = (zone_low + zone_high) / 2
    elif entry.get("last_event"):
        planned_entry = float(entry["last_event"].get("price", price))
    else:
        planned_entry = price

    current_risk = abs(price - invalidation)
    planned_risk = abs(planned_entry - invalidation)
    current_validated_targets = build_validated_targets(
        structural_candidates, price, current_risk
    )
    planned_validated_targets = build_validated_targets(
        structural_candidates, planned_entry, planned_risk
    )
    primary_target = pick_primary_structural_target(structural_candidates)

    structural_rr = compute_structural_rr(planned_entry, invalidation, primary_target)
    structural_rr_quality = compute_structural_rr_quality(structural_rr, primary_target)
    mechanical_rr = compute_mechanical_rr(planned_entry, invalidation, planned_validated_targets)
    # TP1 / nearest planned objective is the actionable risk/reward test.
    # Current-price R:R is retained separately so the execution layer can
    # distinguish a good setup from a bad current entry.
    actionable_rr = float(planned_validated_targets[0]["r"]) if planned_validated_targets else 0.0
    current_actionable_rr = float(current_validated_targets[0]["r"]) if current_validated_targets else 0.0

    setup_quality = compute_setup_quality(setup_type, regime_info, structural_rr,
                                           structural_rr_quality, planned_validated_targets)
    ...
    entry_quality = compute_entry_quality(extension_ratio_pct, extension_status,
                                           current_validated_targets, price, invalidation, setup_type)
    ...
    return {
        ...
        "actionable_rr": actionable_rr,
        "current_actionable_rr": current_actionable_rr,
        ...
        "validated_targets": planned_validated_targets,
        "setup_quality": setup_quality,
        "entry_quality": entry_quality,
        "structural_rr_quality": structural_rr_quality,
        "invalidation_level": invalidation,
        "invalidation_reason": invalidation_reason,
        "invalidation_timeframe": entry.get("_last_invalidation_timeframe", ENTRY_TF),
...
#
# NOTE on invalidation_timeframe: Phase 3 now prepares 1H structural
# candidates anchored to the same directional execution zone, and the
# invalidation helpers record whether the selected structural source is 15M
# or 1H. The reported timeframe therefore reflects the actual source rather
...
def determine_execution_state(plan, tf_results, direction):
    ...
    PRECEDENCE (audited, fixed in Phase 4.5 -- see report):
      ...
      2. NO_TRADE thesis-quality/structural-R:R gates
    ...
    entry = tf_results.get(ENTRY_TF)
    if not isinstance(entry, dict) or entry.get("price") is None:
        return {"status": "NO_TRADE", "execution_type": None,
                "no_trade_code": "MISSING_ENTRY_TIMEFRAME_DATA",
                "no_trade_reason": f"Missing usable {ENTRY_TF} timeframe data."}
    price = float(entry["price"])
    ...
    out = {
        ...
        "planned_actionable_rr": plan.get("actionable_rr"),
        "current_actionable_rr": plan.get("current_actionable_rr"),
    }
    ...
    # ---- 2. NO_TRADE thesis gates ----
    # IMPORTANT: the 80-point score is a READY-tier evidence floor, not a
    # prerequisite for a legitimate WAITING setup. A good pullback thesis
    # can score 60-79 while price is still away from its planned zone.
    # This is what lets the scanner express: GOOD SETUP / BAD CURRENT PRICE.
    setup_score = plan.get("setup_score")
    if setup_score is not None and setup_score < MIN_SETUP_SCORE:
        out.update({
            "status": "NO_TRADE",
            "execution_type": None,
            "no_trade_code": "SETUP_SCORE_BELOW_DISCOVERY_FLOOR",
            "no_trade_reason": (
                f"Setup score {setup_score:.0f} is below discovery floor {MIN_SETUP_SCORE}."
            ),
        })
        return out

    if plan.get("trend_alignment") == "COUNTERTREND" and not plan.get("reversal_confirmation"):
        out.update({
            "status": "NO_TRADE",
            "execution_type": None,
            "no_trade_code": "COUNTERTREND_WITHOUT_REVERSAL_CONFIRMATION",
            "no_trade_reason": "Setup opposes the HTF regime and lacks qualifying 4H/1H reversal confirmation.",
        })
        return out

    quality_fail = setup_quality < MIN_SETUP_QUALITY
    # actionable_rr is calculated from the planned execution price (zone
    ...
    if quality_fail or rr_fail:
        ...
        if quality_fail:
            reasons.append(
                f"setup quality {setup_quality:.0f} < minimum {MIN_SETUP_QUALITY}"
            )
        if rr_fail:
            reasons.append(
        ...
        if quality_fail and rr_fail:
            ...
            code = "SETUP_QUALITY_BELOW_FLOOR"
        else:
            code = "STRUCTURAL_RR_BELOW_FLOOR"
        out.update({
            "status": "NO_TRADE",
            "execution_type": None,
            "no_trade_code": code,
            "no_trade_reason": "No-trade gate: " + "; ".join(reasons) + ".",
        })
        return out
    ...
    # ---- 4. Entry-location calculation ----
    # Recompute from the entry data rather than relying on a local variable
    # from build_entry_plan(). This keeps the classifier self-contained and
    # safe for every caller.
    directional_zone = _best_directional_zone(entry, direction, max_atr=1.5)
    if directional_zone:
        preferred_entry = (directional_zone["low"] + directional_zone["high"]) / 2
    elif setup_type == "TREND_PULLBACK":
    ...
    # ---- 5. READY_MARKET / READY_LIMIT / NEAR_READY / WAIT_PULLBACK ----
    ready_gate = (setup_score is None or setup_score >= MIN_READY_SCORE) and \
                 setup_quality >= MIN_READY_SETUP_QUALITY \
    ...
    if ready_gate:
        ...
        current_rr = float(plan.get("current_actionable_rr", plan.get("actionable_rr", 0.0)) or 0.0)
        if extension_status == "NOT_EXTENDED" and essentially_at_price and current_rr >= MIN_STRUCTURAL_RR:
...
def update_setup_lifecycle(symbol, plan, scan_cycle=None):
    ...
    snapshot = {
        ...
        # True means an eligible Telegram notification was attempted but
        # delivery has not yet been confirmed. A later scan must retry it.
        "notification_pending": bool((states.get(symbol) or {}).get("notification_pending", False)),
    }
    ...
    # A previously failed Telegram delivery is itself actionable: retry on
    # the next scan even when the setup has not materially changed.
    if old.get("notification_pending"):
        snapshot["notification_pending"] = True
        return {"send_alert": True, "lifecycle": lifecycle,
                "reason": "retrying previously failed Telegram delivery",
...
def record_setup_alert_result(symbol, delivered):
    """Record the delivery outcome for the current setup lifecycle.

    Lifecycle state is persisted before notification delivery so scan state
    remains durable. If Telegram fails, retain a pending flag so the next
    scan retries the same alert instead of silently suppressing it forever.
    """
    states = load_setup_states()
    entry = states.get(symbol)
    if not entry:
        return False
    entry["notification_pending"] = not bool(delivered)
    states[symbol] = entry
    save_setup_states(states)
    return True
...
def _notify_score_update(entry):
    ...
    if entry.get("exchange_sync_status") != "OPEN":
        return
    if not TELEGRAM_ENABLED:
        return
...
def _r_multiple(direction, price, entry_price, invalidation):
    try:
        price, entry_price, invalidation = float(price), float(entry_price), float(invalidation)
    except (TypeError, ValueError):
        return 0.0
    risk = abs(entry_price - invalidation)
    if risk <= 0:
        return 0.0
    return ((price - entry_price) if direction == "BULLISH" else (entry_price - price)) / risk


def _position_r_basis(entry):
    ep = entry.get("exchange_filled_entry_price")
    if ep is not None:
        try:
            ep, risk = float(ep), float(entry.get("exchange_original_risk") or 0)
            if ep > 0 and risk > 0:
                return ep, ep - risk if entry["direction"] == "BULLISH" else ep + risk
        except (TypeError, ValueError):
            pass
    try:
        return float(entry["entry_price"]), float(entry["original_invalidation"])
    except (TypeError, ValueError):
        return None, None


def _target_r_from_position(entry, target_price):
    ep, inv = _position_r_basis(entry)
    return _r_multiple(entry["direction"], target_price, ep, inv) if ep is not None else 0.0


def _freeze_exchange_fill(entry, actual_avg):
    if actual_avg is None or entry.get("exchange_filled_entry_price") is not None:
        return False
    try:
        actual_avg = float(actual_avg)
        risk = abs(actual_avg - float(entry["original_invalidation"]))
        if actual_avg <= 0 or risk <= 0:
            return False
    except (TypeError, ValueError):
        return False
    entry["exchange_filled_entry_price"] = actual_avg
...
def _score_confirmed(tf_results, direction, r_now):
    """Score the health of an already-triggered setup on the same evidence model.

    Active-position health deliberately reuses the current structural evidence
    score so the scanner does not have two competing definitions of conviction.
    Profit progress is an additional health signal, capped so it can improve a
    healthy setup but cannot manufacture conviction after structure deteriorates.
    """
    entry = tf_results.get(ENTRY_TF)
    if not isinstance(entry, dict):
        return 0.0

    base_score, _ = score_setup(tf_results)
    score = float(base_score)

    # A profitable active position gets a modest health bonus, but profit never
    # substitutes for structural evidence.  This keeps the result on a clean
    # 0-100 scale and makes historical/current scores comparable.
    if r_now > 0:
        score += min(r_now * 10.0, 25.0)

    # Preserve a small explicit bonus for the original direction remaining
    # aligned with the live entry-timeframe bias.
    if entry.get("bias") == direction:
        score += 5.0
...
def refresh_entry(entry, active_key):
    ...
    if entry["status"] == "pending":
        ...
        if any_leg_touched:
            ...
            entry["exchange_filled_entry_price"] = None
            entry["exchange_original_risk"] = None
            entry["exchange_fill_frozen_at"] = None
            entry["max_r"] = 0.0
    ...
    if entry["status"] == "triggered":
        ...
        r_entry, r_invalidation = _position_r_basis(entry)
        if r_entry is None:
            entry["last_note"] = "No valid position R-basis; active R suppressed."
            return
        r_now = _r_multiple(direction, price, r_entry, r_invalidation)
        ...
        if direction == "BULLISH":
            ...
            if entry.get("max_favorable_price") is None or price < entry["max_favorable_price"]:
                entry["max_favorable_price"] = price
        if entry.get("exchange_sync_status") == "OPEN":
            entry["max_r"] = max(float(entry.get("max_r", 0.0) or 0.0), float(r_now))
        ...
        if not entry.get("position_targets"):
            entry["position_targets"] = []
            for i, t in enumerate((entry.get("targets") or [])[:3]):
                try:
                    p = float(t["price"])
                except (TypeError, ValueError, KeyError):
                    continue
                entry["position_targets"].append({
                    "price": p,
                    "r": _target_r_from_position(entry, p),
                    "label": t.get("label", f"TP{i+1}"),
                    "target_type": t.get("target_type"),
                    "timeframe_source": t.get("timeframe_source"),
                })
        entry["targets"] = list(entry.get("position_targets") or [])
...
def check_watchlist(active_key):
    ...
    for i, it in enumerate(items):
        ...
        try:
            ...
            elif it["status"] == "triggered" and it.get("exchange_sync_status") == "OPEN":
    ...
    for tf in TFS_ALL:
        ...
        if df is not None:
            try:
                ...
                # A malformed/edge-case market response must never kill the
                # entire multi-market scan. Preserve the missing timeframe and
                # expose the exact failure for diagnosis.
                print(f"  ! {symbol} {tf}: analysis error: {type(exc).__name__}: {exc}")
                result = None
            tf_results[tf] = result
            used[tf] = used_key if result is not None else None
        else:
            tf_results[tf] = None
            used[tf] = None
...
def print_setup(symbol, tf_results, used, score, direction, plan):
    ...
    if status == "NO_TRADE":
        print(f"  NO-TRADE REASON: {plan.get('no_trade_reason', 'No reason recorded -- audit required.')}")
        if plan.get("no_trade_code"):
            print(f"  NO-TRADE CODE: {plan['no_trade_code']}")
...
def _format_setup_alert(symbol, score, direction, plan):
    """Build the one-shot Telegram alert for a newly actionable setup."""
    status = plan.get("status", "UNKNOWN")
    execution = plan.get("execution_type") or "WAIT"
    lines = [
        f"🚨 ATHENA ACTIONABLE SETUP: {symbol} {direction}",
        f"State: {status} | Execution: {execution} | Type: {plan.get('trade_type', 'INTRADAY')}",
        f"Setup Score: {score:.0f}/100 | Setup Quality: {plan.get('setup_quality', 0):.0f} | Entry Quality: {plan.get('entry_quality', 0):.0f}",
        f"Current: {plan.get('current_price', 0):.6g}",
        f"Preferred Entry: {plan.get('preferred_entry', 0):.6g}",
        f"Entry Zone: {plan.get('zone_low', 0):.6g} - {plan.get('zone_high', 0):.6g} ({plan.get('zone_label', 'structure')})",
        f"Invalidation: {plan.get('invalidation_level', plan.get('invalidation', 0)):.6g} ({plan.get('invalidation_timeframe', ENTRY_TF)})",
        f"Structural R:R: {plan.get('structural_rr', 0):.2f}",
    ]
    targets = plan.get("validated_targets") or []
    if targets:
        lines.append("Targets: " + " | ".join(
            f"TP{i+1} {t['price']:.6g} (~{t['r']:.2f}R)" for i, t in enumerate(targets[:3])
        ))
    else:
        lines.append("Targets: NONE detected")
    lines.append("PREPARE — this is actionable now; do not chase if price leaves the execution zone.")
    return "\n".join(lines)


def run_scan(active_key, symbols):
    """Run a full-market scan with bounded I/O concurrency.

    Network candle acquisition is the expensive part of a 150-market scan.
    Each worker handles one symbol's four timeframes sequentially, preserving
    per-symbol ordering/fallback behavior, while up to SCAN_WORKERS symbols are
    fetched and analyzed in parallel. Lifecycle persistence, ranking, and
    Telegram decisions remain serialized in the main thread so shared JSON
    state and notifications cannot race.
    """
    ...
    # Submit per-symbol acquisition + pure SMC analysis to the worker pool.
    # Do NOT move lifecycle or Telegram work into workers: those paths mutate
    # shared JSON state and must remain deterministic/serialized.
    worker_count = max(1, min(SCAN_WORKERS, total))
    print(f"\nScanning {total} symbol(s) with {worker_count} I/O workers...")
    for n, symbol in enumerate(symbols, 1):
        print(f"  Queued {symbol} ({n}/{total})")

    with ThreadPoolExecutor(max_workers=worker_count, thread_name_prefix="athena-scan") as pool:
        futures = {symbol: pool.submit(scan_symbol, active_key, symbol) for symbol in symbols}

        # Process results in original symbol order. This keeps lifecycle state
        # and alert behavior deterministic across scans even though requests
        # complete out of order underneath.
        for n, symbol in enumerate(symbols, 1):
            future = futures[symbol]
            print(f"\nScanning {symbol} ({n}/{total})...")
            try:
                tf_results, used = future.result()
            except Exception as exc:
                # scan_symbol is already defensive, but keep a final boundary
                # here so one worker failure can never abort the whole market run.
                print(f"  ! {symbol}: data scan error: {type(exc).__name__}: {exc}")
                continue
            ...
            try:
                ...
                if score >= MIN_SETUP_SCORE and direction:
                    ...
                    if plan:
                        ...
                        li = plan["lifecycle_info"]
                        if li.get("send_alert") and plan.get("status") in ("READY_MARKET", "READY_LIMIT"):
                            if TELEGRAM_ENABLED:
                                sent = send_telegram_message(_format_setup_alert(symbol, score, direction, plan))
                                record_setup_alert_result(symbol, sent)
                                plan["lifecycle_info"]["telegram_sent"] = bool(sent)
                                if not sent:
                                    print(f"  ! {symbol}: Telegram alert was eligible but delivery failed; queued for retry.")
                            else:
                                plan["lifecycle_info"]["telegram_sent"] = False
                                plan["lifecycle_info"]["telegram_note"] = "Telegram disabled"
                        qualifying.append((symbol, tf_results, used, score, direction, plan))
            except Exception as exc:
                # One pathological symbol must never abort the 150-market scan.
                # Keep the failure visible and continue with the remaining markets.
                print(f"  ! {symbol}: scan error: {type(exc).__name__}: {exc}")

    elapsed = time.monotonic() - scan_started
    avg_market = elapsed / total if total else 0.0
    print(f"\nDone scanning {total} symbol(s). {len(qualifying)} met the setup bar "
          f"(score >= {MIN_SETUP_SCORE}).")
    print(f"SCAN PERFORMANCE: {elapsed:.1f}s total | {avg_market:.2f}s/market | "
          f"{worker_count} I/O workers")
    ...
    for symbol, tf_results, used, score, direction, plan in qualifying:
        try:
            ...
            print(f"  ! {symbol}: print error: {type(exc).__name__}: {exc}")
    ...
    actionable = buckets["READY_NOW"]
    if actionable:
        print("\n" + "!" * 70)
        print("ACTIONABLE PREPARATION — READY NOW")
        print("!" * 70)
        for rank, p in enumerate(actionable, 1):
            li = p.get("lifecycle_info") or {}
            alert_note = ("Telegram SENT" if li.get("telegram_sent") else
                          (li.get("telegram_note") or "Telegram alert suppressed"))
            print(f"#{rank} {p.get('direction')} {p.get('status')} | {p.get('trade_type')} | alert: {alert_note}")
            print(f"   Current: {p.get('current_price', 0):.6g} | Preferred entry: {p.get('preferred_entry', 0):.6g}")
            print(f"   Zone: {p.get('zone_low', 0):.6g} - {p.get('zone_high', 0):.6g} ({p.get('zone_label', 'structure')})")
            print(f"   SL: {p.get('invalidation_level', p.get('invalidation', 0)):.6g} [{p.get('invalidation_timeframe', ENTRY_TF)}]")
            print(f"   SQ/EQ: {p.get('setup_quality', 0):.0f}/{p.get('entry_quality', 0):.0f} | Structural R:R: {p.get('structural_rr', 0):.2f}")
            for i, t in enumerate((p.get('validated_targets') or [])[:3], 1):
                print(f"   TP{i}: {t['price']:.6g} (~{t['r']:.2f}R) [{t.get('target_type', 'STRUCTURE')}]")
        print("!" * 70)

    # Forensic breakdown of why discovery-qualified candidates did not become
    # actionable. This deliberately reports gate failures without changing
    # any execution thresholds. It lets us distinguish weak theses from good
    # theses that are merely waiting for better execution/location.
    no_trade_reasons = {}
    for p in buckets["NO_TRADE"]:
        code = p.get("no_trade_code", "UNKNOWN_NO_TRADE")
        no_trade_reasons[code] = no_trade_reasons.get(code, 0) + 1
    if no_trade_reasons:
        print("\nNO-TRADE FORENSIC BREAKDOWN:")
        labels = {
            "SETUP_SCORE_BELOW_TRADE_FLOOR": "Setup score below trade floor",
            "SETUP_QUALITY_BELOW_FLOOR": "Setup quality below minimum",
            "STRUCTURAL_RR_BELOW_FLOOR": "Actionable TP1 R:R below minimum",
            "COUNTERTREND_WITHOUT_REVERSAL_CONFIRMATION": "Countertrend without reversal confirmation",
            "SETUP_QUALITY_AND_STRUCTURAL_RR_BELOW_FLOOR": "Setup quality AND actionable TP1 R:R below minimum",
            "UNKNOWN_NO_TRADE": "No recorded reason (audit required)",
        }
        for code, count in sorted(no_trade_reasons.items(), key=lambda x: (-x[1], x[0])):
            print(f"  {count:>3}  {labels.get(code, code)}")
        print("  Note: candidates in WAITING/NEAR_READY are structurally acceptable but not currently actionable; NO_TRADE means a hard quality/R:R gate failed.")
    print("#" * 70)
