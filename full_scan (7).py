...
def _sync_watchlist(active_key):
    ...
    for it in items:
        ...
        sync_state = bingx.sync_position(it)

        if sync_state == bingx.SYNC_OPEN:
            bingx.freeze_fill_if_needed(it)
            try:
                scanner.refresh_entry(it, active_key)
                time.sleep(0.1)
            except Exception as exc:
                scanner._log(it, f"Active refresh error, exchange state preserved: {exc}")
            ...
            elif sync_state == bingx.SYNC_CLOSED:
        ...
        if sync_state == bingx.SYNC_CLOSED:
            if not it.get("position_close_reported"):
                message = _format_close_alert(it)
                print("\n" + "-" * 78)
                print("POSITION CLOSED ALERT")
