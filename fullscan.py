...
def _is_auto_add_candidate(score, plan):
    ...
    if score < getattr(scanner, "HIGH_CONVICTION_SCORE", 90):
        return False
...
def _format_alert(symbol, used, score, direction, plan, is_new=True, reason=None):
    ...
    tier = ("🔥 HIGH-CONVICTION" if score >= getattr(scanner, "HIGH_CONVICTION_SCORE", 90)
            else "🟢 TRADEABLE" if score >= getattr(scanner, "MIN_TRADE_SCORE", 80)
            else "🟡 DEVELOPING")
    if is_new:
        header = f"🆕 NEW SETUP: {symbol} {direction} [{plan.get('trade_type', 'INTRADAY')}]"
        score_line = f"Score: {score:.0f}/100 — {tier}"
    else:
    ...
    lines = [
        ...
        "Trade qualification: PASS" if score >= getattr(scanner, "MIN_TRADE_SCORE", 80) else "Trade qualification: FAIL — monitor only",
...
def scan_all(active_key, symbols):
    ...
    for n, symbol in enumerate(symbols, 1):
        ...
        try:
            ...
            plan = scanner.build_entry_plan(tf_results, direction, regime_info, setup_score=score)
            if not plan:
                continue
