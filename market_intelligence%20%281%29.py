...
from market_intelligence_config_adapter import (
    MarketAssetRequests,
    to_asset_requests,
    to_request_bundle,
)
...
class MarketIntelligence:
    ...
    def get_snapshot(
        ...
        include_pools: Optional[List[PoolRequest]] = None,
    ) -> Dict[str, Any]:
        """
        ...
        if pool_requests:
            ...
            for pool_request in pool_requests:
                status, err, obs = self._run_source(
                    ...
                    fetch_fn=lambda req=pool_request: self._fetch_geckoterminal(req),
            ...
            if pool_success:
                source_status[SOURCE_GECKOTERMINAL] = (
                    STATUS_ERROR if pool_errors else STATUS_OK
                )
            elif pool_no_match and not pool_errors:
                source_status[SOURCE_GECKOTERMINAL] = STATUS_NO_MATCH
            else:
                source_status[SOURCE_GECKOTERMINAL] = STATUS_ERROR
    ...
    def get_snapshot_from_config(
        self, asset_config: MarketAssetConfig
    ) -> Dict[str, Any]:
        """Build one snapshot directly from the explicit config contract."""
        requests = to_asset_requests(asset_config)
        return self.get_snapshot(
            requests.identifiers,
            include_etf=requests.etf_request,
            include_pools=requests.pool_requests,
        )

    def get_snapshots_from_config(
        self, config: MarketIntelligenceConfig
    ) -> List[Dict[str, Any]]:
        """Build one independent snapshot per configured asset, preserving order."""
        return [
            self.get_snapshot(
                request.identifiers,
                include_etf=request.etf_request,
...
def _run_tests() -> None:
    ...
    class MarketIntelligenceTests(unittest.TestCase):
        ...
        def test_config_adapter_integrates_one_asset_without_identifier_fallback(self):
            agg = FakeAggregator()
            mi = MarketIntelligence(aggregator=agg, coingecko_client=FakeCoingeckoClient())
            config = MarketAssetConfig(symbol="BTC", coingecko_id="bitcoin")

            snapshot = mi.get_snapshot_from_config(config)
        ...
        def test_config_adapter_integrates_multiple_pools_in_one_aggregate_batch(self):
            agg = FakeAggregator()
            mi = MarketIntelligence(
                aggregator=agg,
                geckoterminal_client=FakeGeckoTerminalClient(),
            ...
            config = MarketAssetConfig(
                symbol="ETH",
                pool_requests=[
                    PoolRequestConfig(network="eth", pool_address="0xabc"),
                    PoolRequestConfig(network="eth", pool_address="0xabc"),
                ],
            )
            ...
            self.assertEqual(snapshot["source_status"][SOURCE_GECKOTERMINAL], STATUS_OK)
            self.assertEqual(len(agg.calls), 1)
            pool_obs = [o for o in agg.calls[0] if o["category"] == CATEGORY_POOL]
            self.assertEqual(len(pool_obs), 2)

        def test_config_adapter_preserves_multiple_asset_order(self):
            agg = FakeAggregator()
            mi = MarketIntelligence(
                aggregator=agg,
                coingecko_client=FakeCoingeckoClient(),
            ...
            config = MarketIntelligenceConfig(
                assets=[
                    ...
                    MarketAssetConfig(symbol="ETH"),
                ]
            )

            snapshots = mi.get_snapshots_from_config(config)

            self.assertEqual([s["symbol"] for s in snapshots], ["BTC", "ETH"])
            self.assertEqual(len(agg.calls), 2)
