...
def _maybe_send_position_intelligence(entry, sync_result, health_snapshot):
    """Build intelligence and alert only on meaningful operating-state changes."""
    ...
    previous = entry.get("position_intelligence_state")
    entry["position_intelligence_state"] = state

    if previous == state:
        return snapshot, False
    if state not in POSITION_INTELLIGENCE_ALERT_STATES:
        return snapshot, False
    if previous is None and state in {"CAUTION", "RECOVERY"}:
        return snapshot, False

    message = _format_position_intelligence_alert(entry, snapshot, previous)
    print("\n" + "-" * 78)
...
def _sync_watchlist(active_key):
    ...
    for it in items:
        ...
        if sync_state == bingx.OPEN:
            ...
            # Report a newly confirmed exchange-open position exactly once.
            # This is lifecycle telemetry only; it does not change SMC setup fields.
            if not it.get("position_open_reported"):
                open_snapshot = position_intel.build_position_intelligence(
                    it,
                    exchange_position=(sync_result or {}).get("position"),
                    health_snapshot=snapshot,
                )
                open_message = _format_position_open_alert(
                    it,
                    open_snapshot,
                    discovered=False,
                )
                ...
                print(open_message)
                print("-" * 78)
                if scanner.send_telegram_message(open_message):
                    print("  Telegram position-open alert sent.")
                    it["position_open_reported"] = True
                else:
                    print("  Telegram position-open alert failed; will retry on the next scan.")
