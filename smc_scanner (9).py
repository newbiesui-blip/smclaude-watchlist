...
MIN_REALISTIC_TARGET_RR = 1.5      # nearest actionable structural target must offer at least this much R for READY
MIN_REALISTIC_TARGET_RR_SOFT = 1.0 # below this, current price is too close to the first obstacle; prefer pullback
MIN_STOP_BUFFER_ATR = 0.50        # stop must clear the execution zone/reference by at least this much ATR
MAX_STOP_BUFFER_ATR = 3.0          # avoid turning a local setup into an arbitrarily wide stop
MIN_MARKET_QUALITY = 55            # below this, context is too choppy/ambiguous to deploy capital
...
ENTRY_ZONE_ATR_BELOW = 0.35        # structural pullback zone extends this far below reference
ENTRY_ZONE_ATR_ABOVE = 0.10        # ...and this far above reference (bullish mirror for bearish)
...
def compute_realistic_target_metrics(validated_targets, primary_target, structural_rr):
    """
    Measure whether the first real obstacle ahead of current price gives the
    trade enough room to be executable. This deliberately sits alongside
    structural_rr: a distant 1D/4H target may make the thesis look like 10R+,
    while an intervening 15M/1H liquidity level at 0.6R makes the current
    entry unattractive. The nearer obstacle is therefore the realism check;
    the major structural target remains the thesis target.
    """
    if not validated_targets:
        return {"realistic_target_rr": 0.0, "realistic_target_quality": 0,
                "realistic_target": None,
                "realistic_target_reason": "No actionable structural target remains ahead of current price."}

    first = validated_targets[0]
    rr = float(first.get("r", 0.0))
    if rr >= 2.0:
        quality = 100
    elif rr >= 1.5:
        quality = 85
    ...
    primary_note = ""
    if primary_target and first.get("price") != primary_target.get("price"):
        primary_note = (
            f" First obstacle is {rr:.2f}R before the primary structural target "
            f"at {primary_target['price']:.6g}; distant target is not allowed to "
            f"inflate current-entry quality."
        )
    return {
        "realistic_target_rr": round(rr, 2),
        "realistic_target_quality": quality,
        "realistic_target": first.get("price"),
        "realistic_target_reason": (
            f"Nearest actionable structural target offers {rr:.2f}R." + primary_note
        ),
    }
...
def compute_setup_quality(setup_type, regime_info, structural_rr, structural_rr_quality,
                           validated_targets, realistic_target_quality=0):
    """
    ...
    # Structural R:R is only allowed to contribute its full score when the
    # first actionable target is itself realistic. This prevents a distant
    # 1D/4H target (e.g. 10R+) from making a trade look elite when an
    # intervening 15M/1H obstacle sits at 0.6R.
    rr_component *= 0.20 + (realistic_target_quality / 100) * 0.80

    if not validated_targets:
        target_component = 0
    else:
        # Reward real target quality, but do NOT let a distant primary target
        # fully compensate for a nearby obstacle with poor current-entry R:R.
        ...
        target_component *= 0.70 + (realistic_target_quality / 100) * 0.30

    score = min(100, regime_component + setup_component + rr_component + target_component)
    # A first actionable target below 1.5R means the current entry is not a
    # high-quality deployment point, even if a distant structural target is
    # excellent. Keep that distinction explicit in setup quality so the
    # scanner cannot label a 0.6R-obstacle trade as elite.
    if realistic_target_quality < 85:
        score = min(score, 79)
    if realistic_target_quality < 65:
        score = min(score, 69)
    return round(score)
...
def _build_structural_entry_zone(reference_entry, direction, atr):
    """
    Build an execution zone around the setup's structural reference rather
    than around the drifting current price.
    """
    if reference_entry is None or reference_entry <= 0:
        return None
    if atr is not None and atr > 0:
        if direction == "BULLISH":
            low = reference_entry - ENTRY_ZONE_ATR_BELOW * atr
            high = reference_entry + ENTRY_ZONE_ATR_ABOVE * atr
        else:
            low = reference_entry - ENTRY_ZONE_ATR_ABOVE * atr
            high = reference_entry + ENTRY_ZONE_ATR_BELOW * atr
        return min(low, high), max(low, high)
    band = reference_entry * 0.0015
    return reference_entry - band, reference_entry + band


def _targets_beyond_price(structural_targets, direction, price):
    """Keep only structural targets that remain ahead of current price."""
    if not structural_targets:
        return []
    if direction == "BULLISH":
        return [t for t in structural_targets if t["price"] > price]
    return [t for t in structural_targets if t["price"] < price]


def _ensure_structural_stop_geometry(entry, direction, zone_low, zone_high, invalidation, reference_entry, atr):
    """
    Enforce the institutional stop rule: invalidation must sit OUTSIDE the
    execution area with enough volatility room to survive ordinary noise.

    We never move a stop merely to improve R:R. If the nearest structural
    swing is too close, walk to the next qualifying swing. If no genuine
    structural anchor can provide the required room, return None and let the
    caller reject the setup rather than manufacture a stop.
    """
    if atr is None or atr <= 0:
        return invalidation

    if direction == "BULLISH":
        required = zone_low - MIN_STOP_BUFFER_ATR * atr
        if invalidation < required:
            return invalidation
        candidates = sorted(
            (p for p in entry.get("swing_low_prices", []) if p < zone_low),
            reverse=True,
        )
        for p in candidates:
            candidate = p * (1 - STRUCT_BUFFER_PCT / 100)
            if candidate < required:
                distance_atr = (reference_entry - candidate) / atr
                if distance_atr <= MAX_STOP_BUFFER_ATR:
                    return candidate
        # A 20-bar extreme is a real observed structural extreme, but only
        # use it when it actually clears the execution zone and isn't absurdly far.
        extreme = float(entry["df"]["low"].tail(20).min())
        candidate = extreme * (1 - STRUCT_BUFFER_PCT / 100)
        if candidate < required and (reference_entry - candidate) / atr <= MAX_STOP_BUFFER_ATR:
            return candidate
        return None

    required = zone_high + MIN_STOP_BUFFER_ATR * atr
    if invalidation > required:
        return invalidation
    candidates = sorted(
        (p for p in entry.get("swing_high_prices", []) if p > zone_high),
    )
    for p in candidates:
        candidate = p * (1 + STRUCT_BUFFER_PCT / 100)
        if candidate > required:
            distance_atr = (candidate - reference_entry) / atr
            if distance_atr <= MAX_STOP_BUFFER_ATR:
                return candidate
    extreme = float(entry["df"]["high"].tail(20).max())
    candidate = extreme * (1 + STRUCT_BUFFER_PCT / 100)
    if candidate > required and (candidate - reference_entry) / atr <= MAX_STOP_BUFFER_ATR:
        return candidate
    return None


def compute_market_quality(tf_results, direction):
    """
    Deterministic approximation of the prompt's MARKET QUALITY + LOCATION
    gate. Scores whether capital is being deployed into a clean environment,
    not merely whether a local pattern exists.

    100 = clean directional/structural environment; low scores indicate
    conflicting HTFs, range-middle location, stale structure, or no liquidity
    event. This is deliberately conservative: it can veto a setup but never
    creates one.
    """
    d1, h4, h1, entry = (tf_results.get("1D"), tf_results.get("4H"),
                         tf_results.get("1H"), tf_results.get(ENTRY_TF))
    if entry is None:
        return {"market_quality": 0, "market_quality_reason": "No entry-timeframe data."}

    score = 50
    reasons = []
    dir_word = "BULLISH" if direction == "BULLISH" else "BEARISH"

    # Clean HTF agreement is the strongest positive context.
    htf = [r["bias"] for r in (d1, h4, h1) if r and r["bias"] in ("BULLISH", "BEARISH")]
    if len(htf) >= 2 and all(b == dir_word for b in htf):
        score += 30
        reasons.append("HTFs aligned")
    elif d1 and h4 and d1["bias"] != h4["bias"] and d1["bias"] in ("BULLISH", "BEARISH") and h4["bias"] in ("BULLISH", "BEARISH"):
        score -= 30
        reasons.append("1D/4H conflict")
    elif h4 and h4["bias"] == dir_word:
        score += 15
        reasons.append("4H aligned")
    else:
        score -= 10
        reasons.append("weak HTF alignment")

    # 1H equilibrium is a poor deployment location when there is no fresh
    # directional event. Use the recent range only as a location diagnostic.
    if h1 and h1.get("df") is not None and len(h1["df"]) >= 20:
        w = h1["df"].tail(40)
        hi, lo = float(w["high"].max()), float(w["low"].min())
        span = hi - lo
        pos = (float(entry["price"]) - lo) / span if span > 0 else 0.5
        middle = 0.35 <= pos <= 0.65
        no_event = not h1.get("fresh_break") and not h1.get("recent_sweep")
        if middle and no_event:
            score -= 25
            reasons.append("1H range equilibrium/chop")
        elif direction == "BULLISH" and pos < 0.35:
            score += 10
            reasons.append("bullish discount location")
        elif direction == "BEARISH" and pos > 0.65:
            score += 10
            reasons.append("bearish premium location")

    # A fresh break or sweep is evidence of actual participation/displacement.
    if entry.get("fresh_break"):
        score += 10
        reasons.append("fresh entry-TF structure break")
    if entry.get("recent_sweep") and entry["recent_sweep"]["direction"] == ("bullish" if direction == "BULLISH" else "bearish"):
        score += 5
        reasons.append("supporting liquidity sweep")

    score = max(0, min(100, round(score)))
    return {"market_quality": score, "market_quality_reason": "; ".join(reasons) or "mixed market conditions"}


def build_entry_plan(tf_results, direction, regime_info=None, setup_score=None):
    """
    Build the trade thesis and execution geometry separately.

    Structural identity is anchored to the setup's reference level
    (break/sweep/swing/zone), not to current price. This prevents the entry,
    structural target and structural R:R from drifting as price runs away.

    A good thesis that is temporarily too far from its preferred entry is a
    WAIT/PULLBACK state, not a new setup and not an invalidation.
    """
    entry = tf_results.get(ENTRY_TF)
    if entry is None:
        return None
    ...
    # Establish the original structural reference before calculating
    # invalidation, targets, quality or execution distance.
    zone_here = (
        zone
        if (zone
            and zone["direction"] == dir_word
            and pct_distance(price, zone) <= ZONE_PROXIMITY_PCT * 2)
        else None
    )

    if setup_type == "TREND_PULLBACK":
        if zone_here:
            reference_entry = (zone_here["low"] + zone_here["high"]) / 2
        else:
            swing_prices = (
                entry["swing_low_prices"]
                if direction == "BULLISH"
                else entry["swing_high_prices"]
            )
            valid_swings = [
                p for p in swing_prices
                if pct_distance(price, {"low": p, "high": p})
                <= ZONE_PROXIMITY_PCT * 2
            ]
            reference_entry = valid_swings[0] if valid_swings else price
    else:
        last_event = entry.get("last_event")
        reference_entry = last_event["price"] if last_event else price

    # A real nearby OB/FVG remains the zone for a genuine pullback setup.
    # Otherwise use an ATR-sized structural execution zone around the
    # original reference -- never around wherever current price drifted.
    if zone_here and setup_type == "TREND_PULLBACK":
        zone_low, zone_high = zone_here["low"], zone_here["high"]
        zone_lbl = zone_label(zone_here)
    else:
        built_zone = _build_structural_entry_zone(reference_entry, direction, atr)
        if built_zone is None:
            return None
        zone_low, zone_high = built_zone
        zone_lbl = "structural execution zone"

    # Invalidation is tied to the original structural geometry, then checked
    # against the execution area. A stop inside/just beyond the entry zone is
    # not treated as good risk management: it is normal-noise territory.
    if zone_here and setup_type == "TREND_PULLBACK":
        invalidation = find_structural_invalidation(entry, direction, zone_here)
    else:
        invalidation = find_structural_invalidation_no_zone(
            entry, direction, reference_entry
        )
    invalidation = _ensure_structural_stop_geometry(
        entry, direction, zone_low, zone_high, invalidation, reference_entry, atr
    )
    if invalidation is None:
        return None
    invalidation_reason = (
        f"Structural invalidation beyond the execution area "
        f"{zone_low:.6g}-{zone_high:.6g}, with at least {MIN_STOP_BUFFER_ATR:.2f} ATR "
        f"of noise room; thesis fails only if the underlying structural anchor breaks."
    )

    # Legacy actionable targets remain available for existing consumers.
    targets = find_tp_targets(entry, direction, price, invalidation)

    alignment = regime_info["trend_alignment"] if regime_info else None

    # Structural targets are locked to the original reference price. This
    # means the target cannot jump farther away merely because current price
    # crossed the original liquidity.
    structural_candidates = gather_structural_targets(
        tf_results, direction, reference_entry, alignment
    )
    primary_target = pick_primary_structural_target(structural_candidates)

    risk = abs(reference_entry - invalidation)
    actionable_targets = _targets_beyond_price(
        structural_candidates, direction, price
    )
    current_risk = abs(price - invalidation)
    validated_targets = build_validated_targets(
        actionable_targets, price, current_risk
    )

    structural_rr = compute_structural_rr(
        reference_entry, invalidation, primary_target
    )
    structural_rr_quality = compute_structural_rr_quality(
        structural_rr, primary_target
    )
    realistic_target_info = compute_realistic_target_metrics(
        validated_targets, primary_target, structural_rr
    )
    mechanical_rr = compute_mechanical_rr(
        price, invalidation, validated_targets
    )

    setup_quality = compute_setup_quality(
        setup_type, regime_info, structural_rr, structural_rr_quality,
        actionable_targets, realistic_target_info["realistic_target_quality"]
    )
    market_quality_info = compute_market_quality(tf_results, direction)
    ...
    entry_quality = compute_entry_quality(
        extension_ratio_pct, extension_status, validated_targets,
        price, invalidation, setup_type
    )

    # Trade horizon uses original structural risk, so it doesn't shrink as
    # price runs and falsely turn a developed setup into a scalp.
    trade_type = classify_trade_type(
        reference_entry,
        invalidation,
        [
            {"r": abs(t["price"] - reference_entry) / risk if risk > 0 else 0}
            for t in structural_candidates
        ],
    )
    trade_type = refine_trade_type(
        trade_type, primary_target, tf_results, direction
    )
    ...
    return {
        ...
        "market_quality": market_quality_info["market_quality"],
        "market_quality_reason": market_quality_info["market_quality_reason"],
        "trade_type": trade_type,
        "mechanical_rr": mechanical_rr,
        "structural_rr": structural_rr,
        "realistic_target_rr": realistic_target_info["realistic_target_rr"],
        "realistic_target": realistic_target_info["realistic_target"],
        "realistic_target_quality": realistic_target_info["realistic_target_quality"],
        "realistic_target_reason": realistic_target_info["realistic_target_reason"],
        ...
        "structural_target_reason": (
            primary_target["target_reason"] if primary_target
            else "No meaningful structural target found beyond the original reference."
        ),
        "structural_targets": structural_candidates,
...
def determine_execution_state(plan, tf_results, direction):
    ...
    atr = compute_atr(entry.get("df"))
    risk = abs(price - invalidation)
    current_high = float(entry["df"]["high"].iloc[-1]) if entry.get("df") is not None else price
    current_low = float(entry["df"]["low"].iloc[-1]) if entry.get("df") is not None else price
    ...
    market_quality = plan.get("market_quality", 100)
    if market_quality < MIN_MARKET_QUALITY:
        out.update({"status": "NO_TRADE", "execution_type": None,
                    "no_trade_reason": (
                        f"Market quality {market_quality:.0f}/100 is below "
                        f"deployment threshold {MIN_MARKET_QUALITY}: {plan.get('market_quality_reason', '')}"
                    )})
        return out
    ...
    # A large distant structural R:R cannot rescue a poor current entry.
    # If the first actionable market obstacle is under 1R, the setup may still
    # be valid after a pullback, but it is not deployable at the current price.
    realistic_rr = plan.get("realistic_target_rr", 0.0)
    if realistic_rr < MIN_REALISTIC_TARGET_RR_SOFT:
        out.update({
            "status": "WAIT_PULLBACK", "execution_type": None,
            "no_trade_reason": plan.get("realistic_target_reason", "Insufficient realistic target room."),
        })
        return out
    ...
    # ---- 4. Entry-location calculation ----
    # The preferred entry is the structural reference captured while the
    # thesis formed. It must not drift with current price.
    preferred_entry = plan.get(
        "reference_entry",
        (plan["zone_low"] + plan["zone_high"]) / 2
    )
    ...
    ready_gate = (setup_quality >= MIN_READY_SETUP_QUALITY
                  ...
                  and plan.get("realistic_target_rr", 0.0) >= MIN_REALISTIC_TARGET_RR)
...
def update_setup_lifecycle(symbol, plan, scan_cycle=None):
    ...
    snapshot = {
        ...
        "current_high": plan.get("current_high"),
        "current_low": plan.get("current_low"),
    ...
    # The original structural target is part of the setup thesis. Compare
    # the current candle against the OLD snapshot target before allowing the
    # setup to persist as WAIT_PULLBACK. Once the objective is reached, the
    # original setup is complete; a later reversal is a new setup.
    old_target = old.get("structural_target") if old else None
    old_status = old.get("status") if old else None
    old_terminal = old_status in ("INVALID", "NO_TRADE") or old.get("lifecycle") in (
        "TP_EXIT", "INVALIDATED", "EXPIRED"
    )
    # Invalidation has precedence over target consumption when both are observable in the same OHLC candle.
    # OHLC cannot establish intrabar ordering, so an invalid plan must not be promoted to TP_EXIT.
    if old and old_target is not None and not old_terminal and status != "INVALID":
        current_high = plan.get("current_high", plan.get("current_price"))
        current_low = plan.get("current_low", plan.get("current_price"))
        target_consumed = (
            (old.get("direction") == "BULLISH" and current_high is not None
             and current_high >= old_target)
            or
            (old.get("direction") == "BEARISH" and current_low is not None
             and current_low <= old_target)
        )
        if target_consumed:
            same_lineage = (
                old.get("setup_type") == setup_type
            ...
            if same_lineage:
                ...
                return {
                    ...
                    "lifecycle": "TP_EXIT",
                    "reason": (
                        f"original structural target {old_target:.6g} was "
                        f"reached/consumed; original setup completed"
                    ),
                    "is_new": False,
                    "fingerprint": fingerprint,
                    "completed": True,
            ...
            snapshot["completion_of_previous"] = (
                f"previous target {old_target:.6g} consumed"
            )
            ...
            return {
                ...
                "lifecycle": lifecycle,
                "reason": (
                    f"new setup detected after previous structural target "
                    f"{old_target:.6g} was consumed"
                ),
                "is_new": True,
                "fingerprint": fingerprint,
            }
...
def refresh_entry(entry, active_key):
    ...
    if entry["status"] == "pending":
        ...
        # Invalidation has absolute precedence over trigger detection.
        # A price at/beyond the stop must never be treated as a valid fill,
        # even if it is also beyond a pending entry leg.
        if breached:
...
def print_setup(symbol, tf_results, used, score, direction, plan):
    ...
    print(f"MARKET QUALITY: {plan.get('market_quality', 'n/a')}/100  {plan.get('market_quality_reason', '')}")
    print("-" * 70)
    ...
    for n, symbol in enumerate(symbols, 1):
        ...
        if score >= MIN_SETUP_SCORE and direction:
            ...
            if plan:
                ...
                if plan["lifecycle_info"].get("completed"):
                    plan["status"] = "COMPLETED"
                    plan["execution_type"] = None
                    plan["completion_reason"] = plan["lifecycle_info"]["reason"]
