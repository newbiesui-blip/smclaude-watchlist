...
def _bingx_history(symbol, start_ms):
    ...
    try:
        data = _bingx_signed_get("/openApi/swap/v2/trade/allFillOrders", {"tradingUnit": "COIN", "startTs": start_ms, "endTs": end_ms, "symbol": symbol})
...
def add_to_watchlist(symbol, exchange_key, score, direction, plan,
    ...
    items.append({
        ...
        "exchange_unrealized_pnl": None, "exchange_liquidation_price": None,
...
def _notify_status_change(entry, prev_status):
    ...
    if new_status == "triggered":
        ...
        cpt = f" at {cp:.6g}" if cp is not None else ""
        lines = [
            f"\U0001F534 POSITION CLOSED: {symbol} {direction} [{trade_type}]",
            f"Reason: {reason}{cpt}",
...
def _notify_position_health_transition(entry):
    """Alert only on a position-health state transition. Never an entry signal."""
    ...
    lines = [
        ...
        "Existing confirmed BingX OPEN position only -- NOT a new entry signal.",
    ]
    if current == "EXIT_WARNING":
        ...
        lines.append("Action: CAUTION -- thesis is weakening; monitor next confirmation.")
    elif current == "RECOVERY":
        lines.append("Action: Conditions improving; recovery is not a new entry signal.")
    send_telegram_message("\n".join(lines))
    ...
    if entry["status"] == "triggered":
        ...
        if str(entry.get("exchange_sync_status", "")).upper() == "OPEN":
            ...
            if health_snapshot.get("active"):
                ...
                _log(entry, f"Position health: {health_snapshot['health_state']} | {health_snapshot['reason']}") if health_transition else None
