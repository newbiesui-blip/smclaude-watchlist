...
def bingx_api_symbol(symbol: Any) -> str:
    """Return the symbol format expected by BingX swap API requests."""
...
def _find_closed_history(
    ...
    payload = _signed_get(
        ...
        {
            "symbol": bingx_api_symbol(symbol),
            "startTs": start_ms,
            "endTs": end_ms,
            "pageIndex": 1,
            "pageSize": 100,
...
def confirm_closed_position(
    entry: Dict[str, Any],
    api_key: Optional[str] = None,
    secret: Optional[str] = None,
) -> Optional[Dict[str, Any]]:
    """Return matching position-history evidence for a previously missing position.

    This is read-only. A missing OPEN snapshot is never treated as closure
    unless authoritative positionHistory contains a matching record. For
    fallback identities without an exchange position ID, closure is not
    accepted because symbol/side alone cannot uniquely identify a position.
    """
    ...
    direction = normalize_side(entry.get("side") or entry.get("direction"))
    if direction not in {"LONG", "SHORT"} or not symbol:
        return None

    start_time = entry.get("first_seen_at") or entry.get("opened_at") or entry.get("created_at")
    start_dt = _history_timestamp(start_time)
    start_ms = (
        int(start_dt.timestamp() * 1000)
        if start_dt is not None
        else int(time.time() * 1000) - 7 * 24 * 60 * 60 * 1000
    )
    return _find_closed_history(
        symbol, direction, start_ms, entry=entry, api_key=api_key, secret=secret
    )
...
def list_open_positions(
    ...
    """Return every currently OPEN BingX position (read-only).

    Uses the existing authenticated GET path and normalization helpers.

    Returns:
        A list of normalized position records. An empty list means the
        account currently has zero open positions.

    Raises:
        RuntimeError / requests exceptions on authentication, network,
        or HTTP failure so callers can distinguish API_ERROR from an
        empty account.

    Each record contains at minimum:
        symbol              (canonical XXXUSDT)
        side                (LONG or SHORT)
        amount              (positive absolute size)
        exchange_position_id (when present)
        avg_entry_price
        mark_price
        unrealized_pnl
        raw                 (original exchange payload)
    """
    payload = _signed_get(
        "/openApi/swap/v2/user/positions",
        {},
    ...
    for position in positions:
        ...
        if side not in {"LONG", "SHORT"}:
            # Fallback: BingX sometimes encodes side via signed amount
            # (already abs'd by _position_amount). Treat unknown as skip.
            continue
...
def _run_discover_open_position_tests():
    ...
    class DiscoverOpenPositionTests(unittest.TestCase):
        ...
        def test_sync_missing_open_and_matching_history_is_closed(self):
            ...
            self.assertEqual(
                entry["position_exit_reason"],
                "TAKE_PROFIT",
            )
            self.assertEqual(
                entry["exchange_close_order_id"],
                "close-001",
            )
            self.assertEqual(
                entry["exchange_realized_pnl"],
                "250",
            )
