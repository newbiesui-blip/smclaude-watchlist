...
def build_entry_plan(tf_results, direction, regime_info=None, setup_score=None):
    ...
    # TP1 / nearest validated target is the actionable risk/reward test.
    # Keep structural_rr as the thesis-level target metric, but never let a
    # distant target alone make a trade executable when the first real
    # objective is too close.
    actionable_rr = float(validated_targets[0]["r"]) if validated_targets else 0.0
...
def determine_execution_state(plan, tf_results, direction):
    ...
    if setup_score is not None and setup_score < MIN_TRADE_SCORE:
        out.update({
            "status": "NO_TRADE",
            "execution_type": None,
            "no_trade_code": "SETUP_SCORE_BELOW_TRADE_FLOOR",
            "no_trade_reason": (
                f"Setup score {setup_score:.0f} is below trade threshold {MIN_TRADE_SCORE}."
            ),
        })
        return out
