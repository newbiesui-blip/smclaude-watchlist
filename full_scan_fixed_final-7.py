...
import athena_notification_state as notification_state
...
def main():
    ...
    for plan in alert_plans:
        ...
        fingerprint = lifecycle.get("fingerprint") or plan.get("setup_fingerprint")
        should_alert, alert_reason = notification_state.claim_opportunity_alert(
            symbol, direction, fingerprint, plan.get("status"), scan_cycle=int(time.time() // 900)
        )
        # Lifecycle is still maintained for setup expiry/lineage, but it is
        # no longer allowed to veto a qualifying WAIT/NEAR opportunity alert.
        # This closes the P0 where an unchanged qualifying setup was visible
        # in final buckets yet produced no Telegram message.
        if should_alert:
            ...
            if getattr(scanner, "TELEGRAM_ENABLED", False):
                ...
                if sent:
                    notification_state.record_opportunity_alert(
                        symbol, direction, fingerprint, plan.get("status"),
                        scan_cycle=int(time.time() // 900)
                    )
                    alerts_sent += 1
                else:
                    print(f"  ! {symbol}: Telegram alert was eligible but delivery failed; it remains retryable.")
            else:
