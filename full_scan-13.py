...
def _format_alert(symbol, used, score, direction, plan, is_new=True, reason=None):
    ...
    if dctx:
        ...
        lines.append(f"Derivatives: {dstate}" + (f" ({dscore:+.0f})" if isinstance(dscore, (int, float)) else ""))
        if dreason:
...
def scan_all(active_key, symbols):
    ...
    # One bulk Bybit derivatives fetch per scan cycle. No L/S calls here:
    # selected-market enrichment will be added only after the base integration
    # is proven stable.
