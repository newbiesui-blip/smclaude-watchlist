...
def _format_alert(symbol, used, score, direction, plan):
    ...
    lines = [
        f"🆕 {symbol} {direction} [{plan.get('trade_type', 'INTRADAY')}] — score {score:.0f}",
...
def main():
    ...
    for plan in ready:
        ...
        if lifecycle.get("send_alert"):
            message = _format_alert(symbol, used, score, direction, plan)
            print("\n" + "-" * 78)
            print("ALERT")
            print(message)
            print("-" * 78)
