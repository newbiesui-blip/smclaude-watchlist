...
import market_environment_runtime as market_env_runtime
...
def _recent_dead_setup_reentry_block(symbol, direction, plan):
    """Return a boundary-level block for a recently failed same-direction setup."""
    try:
        items = scanner.load_watchlist() or []
    except Exception as exc:
        print(f"  ! re-entry history unavailable for {symbol}: {type(exc).__name__}: {exc}")
        return None

    wanted_symbol = str(symbol or "").strip().upper()
    wanted_direction = str(direction or "").strip().upper()
    candidate_fp = plan.get("setup_fingerprint")
    cooldown_hours = float(getattr(scanner, "REENTRY_COOLDOWN_HOURS", 0) or 0)
    history_hours = float(getattr(scanner, "REENTRY_HISTORY_HOURS", 0) or 0)

    for old in items:
        if not isinstance(old, dict):
            continue
        if str(old.get("symbol", "")).strip().upper() != wanted_symbol:
            continue
        if str(old.get("direction", "")).strip().upper() != wanted_direction:
            continue
        if old.get("status") not in ("invalidated", "expired") or not old.get("dead_reported"):
            continue
        dead_at = old.get("invalidated_at") or old.get("expired_at")
        if not dead_at:
            continue
        try:
            age_hours = float(scanner._hours_from_now(dead_at))
        except Exception:
            continue
        if age_hours < 0:
            continue
        if cooldown_hours > 0 and age_hours < cooldown_hours:
            return ("REENTRY_COOLDOWN_ACTIVE", f"same-direction setup failed {age_hours:.2f}h ago (cooldown {cooldown_hours:.2f}h)")
        old_fp = old.get("setup_fingerprint")
        if history_hours > 0 and age_hours < history_hours and candidate_fp and old_fp and str(candidate_fp) == str(old_fp):
            return ("REENTRY_SAME_STRUCTURE", f"same setup structure failed {age_hours:.2f}h ago (history {history_hours:.2f}h)")
    return None


def _apply_reentry_boundary_gate(symbol, direction, plan, lifecycle):
    """Suppress repeat NEW-entry alerts/adds without altering SMC decisions."""
    if not isinstance(lifecycle, dict) or not lifecycle.get("is_new"):
        return lifecycle
    blocked = _recent_dead_setup_reentry_block(symbol, direction, plan)
    if not blocked:
        return lifecycle
    code, detail = blocked
    lifecycle = dict(lifecycle)
    lifecycle.update({"send_alert": False, "is_new": True, "reason": code, "reentry_blocked": True, "reentry_block_reason": detail})
    plan["reentry_blocked"] = True
    plan["reentry_block_reason"] = detail
    print(f"  {symbol} {direction}: NEW setup suppressed by {code} — {detail}")
    return lifecycle
...
def _extract_exchange_opened_at(position):
    ...
    for key in ("openTime", "openTimestamp", "positionOpenTime", "position_opened_at", "openedAt", "open_time", "entryTime", "entry_time", "createTime", "createdAt", "createTimestamp"):
        ...
        if value in (None, ""):
            continue
        parsed = position_intel._timestamp_ms(value)
        if parsed is not None:
            return datetime.fromtimestamp(parsed / 1000, tz=timezone.utc).isoformat()
    return None


def _ensure_position_opened_at(entry, exchange_position=None):
    """Anchor duration to the actual position, not the age of its setup."""
    if entry.get("position_opened_at"):
        return entry["position_opened_at"]
    opened_at = _extract_exchange_opened_at(exchange_position) or datetime.now(timezone.utc).isoformat()
    entry["position_opened_at"] = opened_at
    return opened_at


def _apply_position_duration(entry, snapshot):
    opened_at = entry.get("position_opened_at")
    if not opened_at:
        return snapshot
    start_ms = position_intel._timestamp_ms(opened_at)
    if start_ms is None:
        return snapshot
    duration_ms = max(0, int(datetime.now(timezone.utc).timestamp() * 1000) - start_ms)
    snapshot["duration_ms"] = duration_ms
    snapshot["duration"] = position_intel.format_duration(duration_ms)
    snapshot["position_opened_at"] = opened_at
    return snapshot


def _registry_identity_for_entry(entry):
    position_id = entry.get("exchange_position_id") or entry.get("position_id") or entry.get("bingx_position_id")
    if not position_id or not preg.registry_enabled():
        return None, None
    registry = preg.load_registry()
    wanted = str(position_id).strip()
    for identity, record in registry.get("positions", {}).items():
        if str(record.get("exchange_position_id") or "").strip() == wanted:
            return identity, record
    return None, None


def _mark_registry_open_alert(identity):
    if identity:
        try:
            preg.mark_open_alert_sent(identity)
        except Exception as exc:
            print(f"  ! Could not persist registry OPEN-alert state: {type(exc).__name__}: {exc}")
...
def _format_position_open_alert(entry, snapshot, discovered=False):
    ...
    be = snapshot.get("break_even") or {}
    partial = snapshot.get("partial_profit") or {}
    sl_distance = snapshot.get("sl_distance") or {}
    ...
    entry_price_num = _safe_float(entry_price)
    current_price_num = _safe_float(current_price)
    pnl_num = _safe_float(pnl)
    current_r_num = _safe_float(current_r)
    if entry_price_num is not None:
        lines.append(f"Entry: {entry_price_num:.8g}")
    elif entry_price not in (None, ""):
        lines.append(f"Entry: {entry_price}")
    if current_price_num is not None:
        lines.append(f"Current: {current_price_num:.8g}")
    elif current_price not in (None, ""):
        lines.append(f"Current: {current_price}")
    if pnl_num is not None:
        lines.append(
            f"Unrealized P&L: {pnl_num:+.4f}"
            + (f" ({pnl_source})" if pnl_source else "")
        )
    if current_r_num is not None:
        lines.append(f"R: {current_r_num:+.2f}R")
    elif current_r not in (None, ""):
        lines.append(f"R: {current_r}")
    if duration:
        lines.append(f"Time in trade: {duration}")
    if be:
        if be.get("eligible"):
            lines.append(f"BE guidance: MOVE STOP TO ENTRY — {be.get('reason', '')}")
        elif be.get("status") == "UNKNOWN":
            lines.append(f"BE guidance: UNAVAILABLE — {be.get('reason', '')}")
        else:
            lines.append(f"BE guidance: DO NOT MOVE YET — {be.get('reason', '')}")
    if partial:
        if partial.get("suggest"):
            lines.append(f"Partial guidance: CONSIDER PARTIAL — {partial.get('reason', '')}")
        elif partial.get("status") == "UNAVAILABLE":
            lines.append(f"Partial guidance: UNAVAILABLE — {partial.get('reason', '')}")
        else:
            lines.append(f"Partial guidance: NO PARTIAL YET — {partial.get('reason', '')}")
    if sl_distance:
        pct = sl_distance.get("percent")
        lines.append(f"SL distance: {pct:.2f}%" if isinstance(pct, (int, float)) else "SL distance: unavailable")
    lines.append("Guidance is read-only; ATHENA does not modify the position.")
...
def _maybe_send_position_intelligence(entry, sync_result, health_snapshot):
    ...
    _apply_position_duration(entry, snapshot)
...
def _sync_watchlist(active_key):
    ...
    for it in items:
        ...
        if prev_status != "triggered":
            # A dead setup has already emitted its terminal invalidation/expiry
            # notification. Keep it persisted for re-entry history, but do not
            # refresh and re-announce the same dead setup every scan.
            if it.get("dead_reported") and prev_status in ("invalidated", "expired"):
                continue
            try:
        ...
        if sync_state == bingx.OPEN:
            exchange_position = sync_result.get("position") if isinstance(sync_result, dict) else None
            _ensure_position_opened_at(it, exchange_position)
            dctx = None
            try:
            ...
            if not was_exchange_open:
                registry_identity, registry_record = _registry_identity_for_entry(it)
                if registry_record and registry_record.get("open_alert_sent"):
                    print(f"  {it.get('symbol', '?')}: OPEN alert already reported for exchange position {it.get('exchange_position_id')}; suppressing duplicate.")
                    it["position_open_reported"] = True
                else:
                    message = _format_position_open_alert(
                        it, intel_snapshot, discovered=discovered_open,
                    )
                    print("\n" + "-" * 78)
                    ...
                    if scanner.send_telegram_message(message):
                        it["position_open_reported"] = True
                        _mark_registry_open_alert(registry_identity)
        ...
        if sync_state == bingx.CLOSED:
            if not it.get("position_close_reported"):
                ...
                if scanner.send_telegram_message(message):
                    ...
                    # Keep the closure pending so the next successful cycle
                    # retries the notification. A failed Telegram send must
                    # never be recorded as already reported.
                    print("  Telegram closure alert failed; will retry next cycle.")
            continue
...
def scan_all(active_key, symbols, environment_state=None):
    ...
    for n, symbol in enumerate(symbols, 1):
        ...
        try:
            ...
            # Entry-only environment gate: blocked NEW setups are not fed into
            # lifecycle/deduplication. Existing-position monitoring is outside
            # this path and remains independent of market environment state.
            if environment_state is not None and not environment_state.allowed:
                plan["market_environment"] = environment_state.as_dict()
                plan["lifecycle_info"] = {
                    "send_alert": False,
                    "is_new": True,
                    "reason": "NEW_ENTRY_BLOCKED_BY_MARKET_ENVIRONMENT",
                }
            else:
                lifecycle = scanner.update_setup_lifecycle(symbol, plan, scan_cycle)
                lifecycle = _apply_reentry_boundary_gate(symbol, direction, plan, lifecycle)
...
def main():
    ...
    # Read-only market environment. It is passed only into the SMC NEW-entry
    # path; Position Registry discovery and existing-position monitoring remain
    # independent of it. Source failure is fail-closed for NEW entries only.
    try:
        environment_state = market_env_runtime.load_live_state()
    except Exception as exc:
        environment_state = market_env_runtime.evaluator.EnvironmentState(
            False,
            market_env_runtime.evaluator.BLOCK_NEW_ENTRIES,
            "ENVIRONMENT_DATA_UNAVAILABLE",
        )
        print(
            f"! Market environment unavailable: {type(exc).__name__}: {exc}. "
            "NEW entries will not be permitted this cycle."
        )

    if environment_state is None:
        print("Market environment: BLOCK_NEW_ENTRIES | reason=ENVIRONMENT_DATA_UNAVAILABLE")
    else:
        print(
            f"Market environment: {environment_state.state} | "
            f"allowed_new_entries={environment_state.allowed} | "
            f"reason={environment_state.reason}"
        )

    # Exchange-first Position Registry (feature-flagged).
    # Runs independently of SMC candidate count so orphans are always visible.
    if preg.registry_enabled():
        try:
            # One-time / idempotent migration of already-OPEN watchlist entries
            preg.migrate_from_watchlist()
            recon = preg.reconcile()
            if not recon.get("ok"):
                print(f"! Position registry API issue: {recon.get('error')}")
            else:
                print(
                    f"Position registry: discovered={len(recon['discovered'])} "
                    f"updated={len(recon['updated'])} "
                    f"not_found={len(recon['not_found'])}"
                )

            # Classify SMC-linked vs true orphans BEFORE any notification.
            # SMC-linked positions are handed to legacy _sync_watchlist for
            # OPEN ownership; only genuine orphans use the registry OPEN path.
            classification = preg.classify_smc_links(recon["registry"])
            print(
                f"Position registry links: smc_linked={classification['linked']} "
                f"orphans={classification['orphans']}"
            )
            preg.save_registry(recon["registry"])

            # 0) Confirmed exchange closures are lifecycle events too.
            # Registry closure is based on authoritative BingX position history;
            # send the close alert once and retain retry state if Telegram fails.
            for identity, entry in preg.iter_closed_needing_alert(recon["registry"]):
                ctx = preg.build_minimal_context_for_health(entry)
                for key in (
                    "exchange_close_price", "exchange_closed_at",
                    "exchange_realized_pnl", "exchange_close_source",
                    "position_exit_reason",
                ):
                    if key in entry:
                        ctx[key] = entry[key]
                message = _format_close_alert(ctx)
                print("\n" + "-" * 78)
                print("POSITION CLOSED ALERT (registry)")
                print(message)
                print("-" * 78)
                if scanner.send_telegram_message(message):
                    preg.mark_close_alert_sent(identity)
                    print("  Telegram closure alert sent (registry).")

            # 1) Exactly one OPEN notification for true orphans only.
            for identity, entry in preg.iter_open_needing_alert(
                recon["registry"], orphans_only=True
            ):
                ctx = preg.build_minimal_context_for_health(entry)
                _ensure_position_opened_at(ctx, None)
                orphan_snapshot = position_intel.build_position_intelligence(ctx, exchange_position=entry, health_snapshot=None)
                _apply_position_duration(ctx, orphan_snapshot)
                message = _format_position_open_alert(ctx, orphan_snapshot, discovered=True)
                print("\n" + "-" * 78)
                print("POSITION OPEN ALERT (registry orphan)")
                print(message)
                print("-" * 78)
                if scanner.send_telegram_message(message):
                    preg.mark_open_alert_sent(identity)
                    print("  Telegram position-open alert sent (registry orphan).")

            # 2) Feed existing health + intelligence for registry orphans
            #    so monitoring continues even with empty watchlist / zero SMC.
            for identity, entry in recon["registry"].get("positions", {}).items():
                if entry.get("lifecycle") != bingx.OPEN:
                    continue
                if not entry.get("orphan", True):
                    continue  # SMC-linked: legacy path owns monitoring
                ctx = preg.build_minimal_context_for_health(entry)
                try:
                    dctx = health.get_derivatives_context(ctx.get("symbol"))
                except Exception:
                    dctx = None
                try:
                    snapshot, transitioned = health.apply_health(ctx, dctx)
                    if transitioned:
                        message = _format_health_alert(ctx, snapshot)
                        print("\n" + "-" * 78)
                        print("POSITION HEALTH ALERT (registry orphan)")
                        print(message)
                        print("-" * 78)
                        scanner.send_telegram_message(message)
                    # Reuse existing intelligence + BUG-2 path. The context
                    # object is intentionally separate from the registry entry,
                    # so persist the resulting health/intelligence state back to
                    # the registry. Without this, every Actions run starts an
                    # orphan position with no previous health/intelligence state
                    # and transitions/material moves can never be detected.
                    intel_snapshot, _intel_sent = _maybe_send_position_intelligence(
                        ctx, {"position": entry}, snapshot
                    )
                    for key in (
                        "position_opened_at",
                        "position_health_state",
                        "position_health_previous_state",
                        "position_health_reason",
                        "position_health_updated_at",
                        "position_health_risk_score",
                        "position_health_derivatives_state",
                        "position_health_derivatives_score",
                        "position_health_transition",
                        "position_intelligence_state",
                        "position_intelligence_snapshot",
                        "position_intelligence_alert_snapshot",
                    ):
                        if key in ctx:
                            entry[key] = ctx[key]
                    if intel_snapshot is not None:
                        entry["position_intelligence_snapshot"] = intel_snapshot
                    entry["last_monitoring_at"] = datetime.now(timezone.utc).isoformat()
                except Exception as hexc:
                    print(
                        f"  ! registry orphan health/intel error "
                        f"{entry.get('symbol')}: {type(hexc).__name__}: {hexc}"
                    )

            # Persist orphan monitoring state so the next scheduled run can
            # compare health/intelligence against the previous observation.
            preg.save_registry(recon["registry"])
        except Exception as exc:
            print(f"! Position registry error: {type(exc).__name__}: {exc}")
    ...
    # When the registry flag is on, the legacy path still runs for SMC-linked
    # watchlist entries; the registry owns pure orphan discovery.
    _sync_watchlist(active_key)

    qualifying = scan_all(active_key, symbols, environment_state)
    ...
    for plan in ready:
        ...
        if (
            not plan.get("reentry_blocked")
            and environment_state is not None
            and environment_state.allowed
            and _is_auto_add_candidate(score, plan)
        ):
            try:
            ...
            and not environment_state.allowed
            and _is_auto_add_candidate(score, plan)
        ):
            print(
                f"  {symbol}: NEW entry/watchlist add blocked by market "
                f"environment ({environment_state.reason})."
            )
