...
on every real status change (pending->triggered, ->invalidated, ->expired)
via the same send_telegram_message() already in smc_scanner.py.

Requires smc_scanner.py to sit in the same directory (same repo).
"""
...
def main():
    ...
    if not active_key:
        ...
        sys.exit(0)   # exit 0 on purpose: a transient network miss shouldn't fail the CI job
