...
def migrate_from_watchlist(
    ...
    for it in items:
        ...
        positions_map[identity] = {
            ...
            # Preserve legacy notification state during migration.
            # Otherwise a previously-reported OPEN can be re-alerted after a
            # transient NOT_FOUND followed by exchange re-discovery.
            "open_alert_sent": bool(it.get("position_open_reported")),
...
def classify_smc_links(
    ...
    by_sym_side: Dict[str, List[Dict[str, Any]]] = {}
    for it in items:
        if not isinstance(it, dict):
            continue
        ...
        side = bingx.normalize_side(it.get("exchange_position_side") or it.get("direction") or "")
        # A symbol/side match is only a safe fallback when the watchlist item
        # itself represents a currently OPEN exchange position. Historical or
        # dead setups must never claim a newly-discovered exchange position.
        if (
            sym
            and side in {"LONG", "SHORT"}
            and str(it.get("status", "")).lower() == "triggered"
            and str(it.get("exchange_sync_status", "")).upper() == bingx.OPEN
        ):
            by_sym_side.setdefault(f"{sym}:{side}", []).append(it)
    linked = orphans = 0
